...
def test_identity_before_network_ordering_locked_by_sentinel_remote(tmp_path: Path, foreign: Path):
    ...
    _ember_e33bbd3b13c22910_path = next(parent for parent in Path(__file__).resolve().parents if (parent / 'pyproject.toml').is_file()).joinpath('src', 'ember', 'governance', 'scripts', 'ember_restart', 'source_authority.py')
