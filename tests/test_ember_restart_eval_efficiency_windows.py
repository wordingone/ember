...
def test_refuses_to_overwrite_existing_efficiency_receipt():
 with tempfile.TemporaryDirectory() as tmp:
  root=Path(tmp);windows=root/"windows.json";out=root/"out.json";windows.write_text(json.dumps([{"tokens":1,"seconds":1.0,"peak_memory_bytes":0}]));out.write_text("preserve")
  r=subprocess.run([sys.executable,str(SCRIPT),"--windows",str(windows),"--output",str(out)],text=True,capture_output=True,check=False);assert r.returncode!=0 and out.read_text()=="preserve"
