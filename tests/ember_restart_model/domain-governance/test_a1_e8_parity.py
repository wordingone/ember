...
def test_mint_parity_receipt_matches_real_validator_and_no_overwrite(tmp_path: Path) -> None:
    ...
    module = _module()
    validator = importlib.import_module("src.ember.governance.scripts.r1_e8_validator")
    battery = importlib.import_module("src.ember.governance.scripts.r1_exit_battery")
    ...
    receipt_path = module.mint_parity_receipt(
        ...
        thresholds_path=ROOT / "docs" / "domains" / "governance" / "spec" / "ember02-preregistration-thresholds-v1.json",
        e7_receipt=e7,
    )
