...
class OwnedOpenAiServerTests(unittest.TestCase):
    ...
    def test_development_config_mismatch_stops_before_tokenizer_or_loader(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            development = {
                ...
                "tokenizer_sha256": "c" * 64,
                "server_source_sha256": hashlib.sha256((ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "serve_owned_openai.py").read_bytes()).hexdigest(),
                "tokens_seen": 2048,
    ...
    def test_development_loader_reads_owner_sharded_v5_shared_model_only(self) -> None:
        import src.ember.model.ember_v0_model as model_module
        from src.ember.model.ember_v0_model import RestartDecoderConfig, UnifiedDecoder
    ...
    def __call__(self, candidate: Path, manifest_receipt: dict[str, object]) -> dict[str, object]:
        ...
        payload = {
            ...
            "counter_sha256": hashlib.sha256(
                (ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "parameter_counter.py").read_bytes()
            ).hexdigest(),
