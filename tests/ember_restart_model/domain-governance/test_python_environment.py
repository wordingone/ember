...
SCRIPT = ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "python_environment.py"
...
def test_install_refuses_environment_drift_before_pip(
    ...
    def reject_environment(_manifest: dict, _profile: str | None = None) -> None:
...
def test_platform_profiles_are_closed_and_select_explicitly() -> None:
    manifest = load_manifest()
    assert set(manifest["observed_environment"]) == {"windows", "linux"}
    assert python_environment.select_platform_profile(manifest, "windows")["python_executable_basename"] == "python.exe"
    assert python_environment.select_platform_profile(manifest, "linux")["python_executable_basename"] == "python3"
    with pytest.raises(
        python_environment.EnvironmentContractError,
        match="platform profile is unavailable",
    ):
        python_environment.select_platform_profile(manifest, "darwin")


@pytest.mark.parametrize("profile,wrong_platform", [
    ("windows", "Linux-wrong"),
    ("linux", "Windows-wrong"),
])
def test_each_platform_profile_refuses_wrong_platform(
    monkeypatch: pytest.MonkeyPatch,
    profile: str,
    wrong_platform: str,
) -> None:
    manifest = load_manifest()
    expected = python_environment.select_platform_profile(manifest, profile)
    monkeypatch.setattr(python_environment.platform, "python_implementation", lambda: expected["implementation"])
    monkeypatch.setattr(python_environment.platform, "python_version", lambda: expected["python_version"])
    monkeypatch.setattr(python_environment.platform, "platform", lambda: wrong_platform)
    monkeypatch.setattr(python_environment.sys, "executable", str(Path("bin") / expected["python_executable_basename"]))
    monkeypatch.setattr(python_environment.importlib.metadata, "version", lambda name: expected["pip_version"])
    with pytest.raises(
        python_environment.EnvironmentContractError,
        match="observed environment mismatch",
    ):
...
def test_cli_platform_profile_is_closed() -> None:
    parser = python_environment._parser()
    assert parser.parse_args(["--platform-profile", "linux", "print-install-command"]).platform_profile == "linux"
    with pytest.raises(SystemExit):
        parser.parse_args(["--platform-profile", "darwin", "print-install-command"])
