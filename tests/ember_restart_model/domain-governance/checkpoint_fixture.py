...
def fixture_counter_receipt(candidate: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "counter_sha256": hashlib.sha256((Path(__file__).resolve().parents[3] / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "parameter_counter.py").read_bytes()).hexdigest(),
