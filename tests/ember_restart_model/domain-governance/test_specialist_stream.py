...
class SpecialistStreamTests(unittest.TestCase):
    ...
    def test_checked_in_stream_manifest_opens_and_replays_full_commitment(self) -> None:
        ...
        receipt_path = _STREAM_RECEIPT_AUTHORITY.path
        manifest_bytes = manifest_path.read_bytes()
        receipt_bytes = receipt_path.read_bytes()
        self.assertEqual(hashlib.sha256(manifest_bytes).hexdigest(), _STREAM_MANIFEST_AUTHORITY.expected_sha256)
        self.assertEqual(hashlib.sha256(receipt_bytes).hexdigest(), _STREAM_RECEIPT_AUTHORITY.expected_sha256)
        manifest = json.loads(manifest_bytes)
    ...
    def test_rebuilt_manifest_uses_exact_generator_dependency_bytes(self) -> None:
        ...
        real_source = ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "specialist_semantics.py"
        ...
        with tempfile.TemporaryDirectory(dir=ROOT) as directory:
            ...
            source_path = root / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "specialist_semantics.py"
    ...
    def test_rebuilt_manifest_uses_exact_verifier_dependency_bytes(self) -> None:
        ...
        real_source = ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "verify_capability_record.py"
        real_sha256 = hashlib.sha256(real_source.read_bytes()).hexdigest()
        with tempfile.TemporaryDirectory(dir=ROOT) as directory:
            root = Path(directory) / "fixture"
            tokenizer_path, config_path = self._bounded_stream_fixture(root)
            source_path = root / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "verify_capability_record.py"
    ...
    def test_bound_module_rejects_dynamic_project_import(self) -> None:
        ...
        source_path = ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "build_owned_vision_scenes.py"
    ...
    def test_build_refuses_invalid_bound_record_before_manifest_or_receipt_publication(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=ROOT) as directory:
            root = Path(directory) / "fixture"
            tokenizer_path, config_path = self._bounded_stream_fixture(root)
            generator_path = root / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "build_owned_vision_scenes.py"
    ...
    def test_prepare_execution_selection_binds_closed_receipt_cursor_and_one_chunk_cache(self) -> None:
        ...
        manifest_path = _STREAM_MANIFEST_AUTHORITY.path
        build_receipt_path = _STREAM_RECEIPT_AUTHORITY.path
        stream = self._open_bound(manifest_path)
        ...
        selection = stream.prepare_execution_selection(
            ...
            expected_build_receipt_sha256=_STREAM_RECEIPT_AUTHORITY.expected_sha256,
        )
        self.assertEqual(selection.receipt["stream_manifest_sha256"], _STREAM_MANIFEST_AUTHORITY.expected_sha256)
        self.assertEqual(selection.receipt["stream_build_receipt_sha256"], _STREAM_RECEIPT_AUTHORITY.expected_sha256)
    ...
    def test_open_execution_selection_requires_canonical_build_authority_and_closed_cursor(self) -> None:
        manifest_path = _STREAM_MANIFEST_AUTHORITY.path
        build_receipt_path = _STREAM_RECEIPT_AUTHORITY.path
        build_receipt_sha256 = _STREAM_RECEIPT_AUTHORITY.expected_sha256
        stream = self._open_bound(manifest_path)
        selection = stream.prepare_execution_selection(
            capability="image", selection_rule_id="image_scene_split_train_v1",
    ...
    def test_specialist_remint_allows_only_the_current_config_pin_to_change(self) -> None:
        from remint_specialist_stream import _assert_manifest_remint

        original = {"schema_version": "stream", "model_config": {"path": "configs/ember-restart-3b.json", "sha256": "a" * 64}, "families": {"audio": {"chunks": ["fixed"]}}}
        candidate = {**original, "model_config": {"path": "configs/ember-restart-3b.json", "sha256": "b" * 64}}
        _assert_manifest_remint(original, candidate, current_config_sha256="b" * 64)
        with self.assertRaisesRegex(ValueError, "non-config commitment"):
            _assert_manifest_remint(
                original, {**candidate, "families": {"audio": {"chunks": ["drift"]}}},
    ...
    def test_issue898_specialist_remint_refuses_any_non_declared_field_change(self) -> None:
        ...
        candidate = json.loads(_STREAM_MANIFEST_AUTHORITY.path.read_bytes())
        ...
        for capability, transitions in _ISSUE898_FAMILY_TRANSITIONS.items():
            ...
            for chunk in family["chunks"]:
                chunk["sha256"] = (
                    ("0" if chunk["sha256"][0] != "0" else "1") + chunk["sha256"][1:]
                )
        _assert_manifest_remint(
            original,
            candidate,
            current_config_sha256=candidate["model_config"]["sha256"],
        )
        candidate["families"]["audio"]["token_count"] += 1
        with self.assertRaisesRegex(ValueError, "non-config commitment"):
            _assert_manifest_remint(
                original,
                candidate,
    ...
    def test_specialist_remint_receipt_keeps_the_no_capability_boundary(self) -> None:
        from remint_specialist_stream import _assert_build_receipt_matches_manifest

        manifest = {
            "lineage": "NEW_PREREGISTERED_STREAM", "data_class": "SEMANTIC_PRETRAINING",
            "corpus_root_sha256": "c" * 64, "range": {"record_count_per_family": 1},
            "families": {"audio": {"record_count": 1, "token_count": 15, "serialized_bytes": 20}},
        }
        raw = json.dumps(manifest, sort_keys=True, separators=(",", ":")).encode() + b"\n"
        receipt = {
            "schema_version": "ember-owned-specialist-stream-build-receipt-v1", "result": "MEASURED",
            "boundary": "STREAM_CONSTRUCTION_NOT_SUFFICIENT_PRETRAINING_OR_CAPABILITY",
            "stream_manifest_sha256": hashlib.sha256(raw).hexdigest(), "lineage": manifest["lineage"],
            "data_class": manifest["data_class"], "corpus_root_sha256": manifest["corpus_root_sha256"],
            "record_count_per_family": 1,
            "families": {"audio": {"records": 1, "tokens": 15, "serialized_bytes_not_materialized": 20}},
            "elapsed_ms": 1,
        }
        _assert_build_receipt_matches_manifest(receipt, manifest_raw=raw, manifest=manifest)
        with self.assertRaisesRegex(ValueError, "does not bind"):
            _assert_build_receipt_matches_manifest(
                {**receipt, "boundary": "CAPABILITY_VERIFIED"}, manifest_raw=raw, manifest=manifest,
            )
