...
PARTITION_TEST = ROOT / "tests" / "ember_restart_model" / "domain-governance" / "test_mint_github_license_partition.py"
...
def test_carrier_b_reopens_historical_rows_through_closed_v2_chain(
    ...
    current_sha = _sha(
        (ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "mint_github_license_partition.py").read_bytes()
    )
    allows_v2 = getattr(
        text_lab_corpus,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "historical_transition_1",
        "historical_transition_2",
        "historical_transition_3",
        "broken_chain",
        "unreachable_cause",
...
def test_partition_authority_refuses_tampered_closed_v2_table(
    ...
    elif mutation.startswith("historical_transition_"):
        ordinal = int(mutation.rsplit("_", 1)[1]) - 1
        table["transitions"][ordinal]["authorization_basis"] += " tampered"
...
def test_partition_plan_mints_v4_packet_and_reopens_partial_authority(tmp_path: Path):
    ...
    assert "license_spdx" not in row
    assert not Path(row["license_partition_receipt"]).is_absolute()
    packet_receipt = output.joinpath(
        *Path(row["license_partition_receipt"]).parts
    )
    assert packet_receipt.is_file()
    assert _sha(packet_receipt.read_bytes()) == row["license_partition_sha256"]
    first_authority_files = {
        path.relative_to(output).as_posix()
        for path in packet_receipt.parent.rglob("*")
        if path.is_file()
    }
    ...
    first_generated = json.loads(first_receipt.read_bytes())["generated_files"]
    assert first_authority_files <= set(first_generated)
    second_corpus = json.loads(
        (second_output / "owned-text-lab-corpus-v4.json").read_bytes()
    )
    second_row = next(
        item for item in second_corpus["sources"]
        if item["source_id"] == "candidate-software_engineering-train-1"
    )
    assert second_row["license_partition_receipt"] == row["license_partition_receipt"]
    assert second_output.joinpath(*Path(row["license_partition_receipt"]).parts).is_file()
