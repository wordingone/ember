...
class OwnedReasoningToolTests(unittest.TestCase):
    ...
    def test_isolated_verifier_accepts_a_batch_of_executed_records(self) -> None:
        ...
        completed = subprocess.run([sys.executable, "-I", str(ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "verify_capability_record.py"), "--records-json-base64", payload], text=True, capture_output=True, timeout=15, check=False)
    ...
    def test_isolated_verifier_accepts_large_record_batch_from_stdin(self) -> None:
        ...
        completed = subprocess.run([sys.executable, "-I", str(ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "verify_capability_record.py"), "--records-json-stdin"], input=json.dumps(records, sort_keys=True, separators=(",", ":")), text=True, capture_output=True, timeout=15, check=False)
        self.assertEqual(completed.returncode, 0, completed.stderr)
