...
class RunnerStorageTests(unittest.TestCase):
    ...
    def test_atomic_create_durable_has_one_cross_process_winner_and_no_temp(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            processes = [
                subprocess.Popen([sys.executable, "-c", worker, str(ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b"), str(target), str(start)], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=environment)
                for _ in range(2)
            ]
            try:
