...
class RemintTextLabInputIdentityTests(unittest.TestCase):
    def _sandbox(self, tmp: Path) -> Path:
        ...
        canonical_tools = Path("src/ember/infrastructure/tools/ember-restart-3b")
        shutil.copytree(ROOT / canonical_tools, root / canonical_tools)
        return root
    ...
    def test_check_fails_for_each_pinned_module_when_edited(self):
        ...
        for name, filename in pinned.items():
            with self.subTest(name=name), tempfile.TemporaryDirectory() as tmp:
                ...
                module = root / "src/ember/infrastructure/tools/ember-restart-3b" / filename
                module.write_bytes(module.read_bytes() + b"\n# drift injected by test\n")
                check = subprocess.run(
                    [sys.executable, "-B", str(root / "src/ember/infrastructure/tools/ember-restart-3b/remint_text_lab_input_identity.py"), "--check"],
                    cwd=root, capture_output=True, text=True,
                )
    ...
    def test_write_cures_the_drift_and_check_then_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = self._sandbox(Path(tmp))
            module = root / "src/ember/infrastructure/tools/ember-restart-3b/run_vertical_slice.py"
            module.write_bytes(module.read_bytes() + b"\n# drift injected by test\n")
            script = root / "src/ember/infrastructure/tools/ember-restart-3b/remint_text_lab_input_identity.py"
