...
def _load_producer():
    path = ROOT / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b" / "mint_issue1719_tranche_admission.py"
...
def _historical_source_fixture(tmp_path: Path):
    ...
    tools = historical / "src" / "ember" / "infrastructure" / "tools" / "ember-restart-3b"
...
def test_historical_predecessor_reopen_binds_reviewed_identity_cure_without_weakening_payload_checks(
    ...
    current_commit = "3" * 40
    resolved_blobs = {
        "src/ember/infrastructure/tools/ember-restart-3b/text_lab_corpus.py": b"resolved text lab\n",
        "src/ember/infrastructure/tools/ember-restart-3b/train.py": b"resolved train\n",
        "src/ember/infrastructure/tools/ember-restart-3b/run_vertical_slice.py": b"resolved vertical\n",
    }
    identity["code_files"] = {
        "text_lab_corpus": _sha(resolved_blobs["src/ember/infrastructure/tools/ember-restart-3b/text_lab_corpus.py"]),
        "train": _sha(resolved_blobs["src/ember/infrastructure/tools/ember-restart-3b/train.py"]),
        "run_vertical_slice": _sha(resolved_blobs["src/ember/infrastructure/tools/ember-restart-3b/run_vertical_slice.py"]),
    }
...
def _source_custody(tmp_path: Path) -> tuple[Path, str, str]:
    ...
    _write(
        custody / "mint-log.json",
        {
            ...
            "adapter_path": "src/ember/infrastructure/tools/ember-restart-3b/text_lab_corpus.py",
