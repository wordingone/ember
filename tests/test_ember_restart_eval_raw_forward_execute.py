...
def test_execute_emits_non_claim_raw_forward_token_ids():
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        source, config, tokenizer, output = root / "model.py", root / "config.json", root / "tokenizer.json", root / "receipt.json"
        ...
        completed = subprocess.run([sys.executable, str(SCRIPT), "--tokenizer", str(tokenizer), "--checkpoint-manifest", str(checkpoint), "--checkpoint-sha256", hashlib.sha256(checkpoint.read_bytes()).hexdigest(), "--output", str(output), "--execute", "--model-source", str(source), "--model-source-sha256", hashlib.sha256(source.read_bytes()).hexdigest(), "--model-config", str(config), "--model-config-sha256", hashlib.sha256(config.read_bytes()).hexdigest(), "--active-expert", "tool", "--input-token-id", "0", "--max-new-tokens", "2", "--device", "cpu"], capture_output=True, text=True)
