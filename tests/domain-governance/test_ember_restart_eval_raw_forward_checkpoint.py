...
def test_records_verified_checkpoint_hash_before_preflight_output():
    with tempfile.TemporaryDirectory() as temporary:
        ...
        completed = subprocess.run([sys.executable, str(SCRIPT), "--tokenizer", str(tokenizer), "--checkpoint-manifest", str(checkpoint), "--checkpoint-sha256", checkpoint_sha256, "--model-config", str(config), "--model-config-sha256", hashlib.sha256(config.read_bytes()).hexdigest(), "--output", str(output)], capture_output=True, text=True)
