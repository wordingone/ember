...
def test_validates_honest_checkpoint_bound_completion_certificate():
 with tempfile.TemporaryDirectory()as temporary:
  ...
  certificate.write_text(json.dumps({'schema_version':'ember-restart-eval-completion-certificate-v1','goal_id':'EMBER-02','workstream_id':'EMBER-02C','checkpoint_manifest_sha256':'a'*64,'evaluator_commit':'b'*40,'benchmarks':[{ 'family': family, 'benchmark_id': family, 'benchmark_version':'1','split_sha256':'c'*64,'protocol_sha256':'d'*64,'command_sha256':'e'*64,'result':'PREFLIGHT_ONLY'} for family in ('text','image','audio','reasoning','code','mathematics','sql','files','browser_ui','terminal','structured_tools')],'comparators':[],'resource_receipts':[],'honest_unresolved_gaps':['v3 checkpoint unavailable'],'next_checkpoint_evaluation':'consume next corrected v3 checkpoint'}))
