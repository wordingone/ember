...
def test_accepts_only_identically_pinned_target_and_comparator_preflights():
 with tempfile.TemporaryDirectory() as tmp:
  ...
  payload=json.loads(out.read_text());assert all(payload[key]==value for key,value in {"result":"COMPARISON_PREFLIGHT","admission":"NOT_ELIGIBLE","capability":"image","benchmark_id":"MMMU","benchmark_version":"v1"}.items())
...
def test_comparator_preflight_receipt_binds_exact_input_bytes():
 with tempfile.TemporaryDirectory() as tmp:
  ...
  comparator.write_bytes(json.dumps({**shared,'subject_checkpoint_sha256':'e'*64,'subject_kind':'open_comparator','comparator_revision':'f'*40,'comparator_size_class':'open_3b'}).encode())
...
def test_comparator_preflight_emits_full_shared_evaluator_tuple():
 with tempfile.TemporaryDirectory() as tmp:
  root=Path(tmp);target=root/'target';comparator=root/'comparator';output=root/'output';shared={'result':'PREFLIGHT_ONLY','admission':'NOT_ELIGIBLE','capability':'text','benchmark_id':'x','benchmark_version':'1','split_sha256':'a'*64,'harness_sha256':'b'*64,'protocol_sha256':'c'*64};target.write_text(json.dumps({**shared,'subject_checkpoint_sha256':'d'*64}));comparator.write_text(json.dumps({**shared,'subject_checkpoint_sha256':'e'*64,'subject_kind':'open_comparator','comparator_revision':'f'*40,'comparator_size_class':'open_3b'}));run=subprocess.run([sys.executable,str(SCRIPT),'--target',str(target),'--comparator',str(comparator),'--output',str(output)],capture_output=True,text=True);assert run.returncode==0,run.stderr;payload=json.loads(output.read_text());assert {key:payload[key] for key in ('split_sha256','harness_sha256','protocol_sha256')}=={key:shared[key] for key in ('split_sha256','harness_sha256','protocol_sha256')}