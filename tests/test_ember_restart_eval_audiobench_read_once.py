...
def test_reads_canonical_predictions_and_closed_run_once_before_hashing(monkeypatch):
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp); predictions = root / "predictions.json"; run = root / "run.json"; score = root / "score.json"
        text = "hello"
        ...
        monkeypatch.setattr(sys, "argv", [str(SCRIPT), "--canonical-predictions", str(predictions), "--run-artifact", str(run), "--score-output", str(score)])
