...
def test_mmmu_rejects_checkpoint_identity_drift_when_frozen_manifest_declares_it(tmp_path):
    ...
    manifest["protocol_sha256"] = module.sha256(f"MMMU:v1:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc:{manifest['scorer']['sha256']}:{manifest['license_sha256']}:{manifest['upstream_tree_git_sha1']}:{manifest['scoring_adapter']['sha256']}:{''}:{manifest['checkpoint_manifest_sha256']}:{manifest['model_config_sha256']}".encode())
