...
Deleting a worktree directory without deregistering it is what strands a registry row, and a stranded row reds the custody census. So every removal path deregisters in the same operation: `retire` records its intent before touching disk, reads back Git's registration afterwards, and writes a dated, reasoned tombstone in the same state write that drops the row. `reconcile --path` does the same for a row whose directory is already gone — cleared, never silently dropped. Run `python scripts/worktree_lifecycle.py audit --strict` to inventory stale registrations, unscannable registrations, unregistered worktree directories and interrupted removals; it is read-only and exits nonzero listing every contradiction it found.
