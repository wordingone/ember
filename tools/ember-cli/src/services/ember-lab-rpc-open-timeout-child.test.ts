...
if (process.env["EMBER_LAB_OPEN_TIMEOUT_CASE"] === "silent") {
  ...
  test("open deadline destroys a silent socket and rejects", async () => {
    ...
    const { callEmberLab } = await import("./ember-lab-rpc.ts?open-timeout-regression");
    await expect(callEmberLab({ pipeName: "\\\\.\\pipe\\ember-lab-open-timeout", requestId: "silent", method: "ping", params: {} }))
      .rejects.toThrow("open retry window elapsed");
    expect(socket?.destroyed).toBe(true);
  }, 12_000);
}

if (process.env["EMBER_LAB_OPEN_TIMEOUT_CASE"] === "late") {
  const { expect, test } = await import("bun:test");
  const net = await import("node:net");
  const { callEmberLab } = await import("./ember-lab-rpc.ts?late-connect-regression");

  test("late pipe availability after the open deadline cannot revive the request", async () => {
    const pipe = `\\\\.\\pipe\\ember-lab-late-connect-${process.pid}-${Math.random().toString(16).slice(2)}`;
    let connections = 0;
    const server = net.createServer((socket) => {
      connections += 1;
      socket.destroy();
    });
    const result = callEmberLab({ pipeName: pipe, requestId: "late", method: "ping", params: {} })
