...
if [ -n "$(git ls-files .claude 2>/dev/null | head -1)" ]; then
  fail ".claude" "agent-harness machinery is tracked; it must be git-ignored"
  git ls-files .claude | sed 's/^/      /' | head -10
else
  ok ".claude" "not tracked"
fi

# ---- 2. no absolute local filesystem paths in tracked text ---------------
# Matches B:/M, B:\M, C:/Users, C:\Users, /mnt/b/, /mnt/c/ and similar.
