...
| F05 | Goal/proof rule | `<public-ember-local-mirror>\GOAL.md` | Requires field-level breakthrough over named prior, broader external/disjoint benchmark, zero-cost verification, deletion/revert sensitivity, and no paid leaderboard/API authority. | Mirror into self-improvement and publication contracts. |
...
| F15 | Agent-loop baseline | `<public-ember-local-mirror>\GOAL.md`, `docs\goal-archive.md`, `scripts\ember_gate_full_parity_harness.py`, `<private-ember-local-checkout>\tools\ember-cli\CLEANROOM-PROTOCOL.md` | Codex goal mode, Claude Code/avir-cli behavior, and Hermes-class agents are comparator substrates for goal execution, CLI parity, tool use, and self-improvement loops. | Absorb into `agent-loop-baselines-v0.md` and `contracts/C8-agent-loop-baselines.md`; keep hidden weight/tool self-edit claims as hypotheses unless sourced. |
