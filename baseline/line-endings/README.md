...
- `<exec-tree>\.gitattributes` has `* text=auto eol=lf`, byte-pins several receipt/config/doc paths with `-text`, and enforces `*.sh` and `*.py` as LF.
- `<public-tree>\.gitattributes` enforces several byte-pinned paths and `*.sh`/`*.py` as LF, but differs from the private checkout and did not explicitly mention `baseline/**`.
