#!/usr/bin/env python3
"""Validate exact adjudication over a v14-filtered LSH candidate index."""
...
VALID_VERDICTS = {
    "C1_CUMULATIVE_FILTERED_LSH_CANDIDATE_INDEX_V14_EXACT_ADJUDICATION_NO_CROSSINGS_NOT_COMPLETION",
    "C1_CUMULATIVE_FILTERED_LSH_CANDIDATE_INDEX_V14_EXACT_ADJUDICATION_CROSSINGS_FOUND_NOT_COMPLETION",
}
...
def main() -> int:
    ...
    parser.add_argument("--expected-skip-buckets", type=int)
    parser.add_argument("--expected-max-buckets", type=int)
    ...
    if args.expected_skip_buckets is not None and receipt.get("index_rows_skipped") != args.expected_skip_buckets:
        failures.append({"code": "skip_bucket_count_mismatch", "actual": receipt.get("index_rows_skipped"), "expected": args.expected_skip_buckets})
    if args.expected_max_buckets is not None and receipt.get("index_rows_adjudicated") != args.expected_max_buckets:
        failures.append({"code": "max_bucket_count_mismatch", "actual": receipt.get("index_rows_adjudicated"), "expected": args.expected_max_buckets})
    if receipt.get("index_row_start_offset") != receipt.get("index_rows_skipped"):
        failures.append({"code": "start_offset_mismatch", "start": receipt.get("index_row_start_offset"), "skipped": receipt.get("index_rows_skipped")})
    if receipt.get("index_row_end_exclusive") != receipt.get("index_row_start_offset", 0) + receipt.get("index_rows_adjudicated", 0):
        failures.append({"code": "end_exclusive_mismatch", "start": receipt.get("index_row_start_offset"), "rows": receipt.get("index_rows_adjudicated"), "end": receipt.get("index_row_end_exclusive")})
    ...
    result = {
        ...
        "verdict": "C1_CUMULATIVE_FILTERED_LSH_CANDIDATE_INDEX_V14_EXACT_ADJUDICATION_VALIDATED" if not failures else "C1_CUMULATIVE_FILTERED_LSH_CANDIDATE_INDEX_V14_EXACT_ADJUDICATION_INVALID",
        "failure_count": len(failures),
        "failures": failures,
        "receipt_path": receipt_rel,
        "summary": {
            "verdict": receipt.get("verdict"),
            "index_rows_skipped": receipt.get("index_rows_skipped"),
            "index_row_start_offset": receipt.get("index_row_start_offset"),
            "index_row_end_exclusive": receipt.get("index_row_end_exclusive"),
