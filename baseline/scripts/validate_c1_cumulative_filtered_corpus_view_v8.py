#!/usr/bin/env python3
"""Validate the cumulative v8 near-duplicate filtered-corpus view receipt.
...
RECEIPT = "receipts/4090-cumulative-filtered-corpus-view-v8-2026-07-01.json"
EXACT_DEDUPE_RECEIPT = "receipts/4090-exact-dedupe-scan-2026-06-30.json"
EXCLUSION_RECEIPT = "receipts/4090-near-duplicate-cumulative-exclusion-manifest-v8-2026-07-01.json"
EXCLUSION_MANIFEST = "fragments/c1-near-duplicate-cumulative-exclusions-v8-2026-07-01.jsonl"
EXPECTED_VERDICT = "C1_CUMULATIVE_FILTERED_CORPUS_VIEW_V8_READY_NOT_COMPLETION"
...
def main() -> int:
    ...
    if filtered.get("excluded_document_count") != 1747 or filtered.get("excluded_token_floor") != 3058613:
        failures.append({"code": "v8_exclusion_totals_do_not_match_required_receipt", "actual_docs": filtered.get("excluded_document_count"), "actual_tokens": filtered.get("excluded_token_floor"), "expected_docs": 1747, "expected_tokens": 3058613})
    if filtered.get("remaining_document_count") != 4234711 or filtered.get("remaining_content_token_floor") != 6974810145:
        failures.append({"code": "v8_remaining_totals_do_not_match_required_receipt", "actual_docs": filtered.get("remaining_document_count"), "actual_tokens": filtered.get("remaining_content_token_floor"), "expected_docs": 4234711, "expected_tokens": 6974810145})
    ...
    result = {
        ...
        "verdict": "C1_CUMULATIVE_FILTERED_CORPUS_VIEW_V8_VALIDATED" if not failures else "C1_CUMULATIVE_FILTERED_CORPUS_VIEW_V8_INVALID",
        ...
        "completion_limit": "This validates a cumulative v8 filtered-corpus view only. It is not an all-pairs near-duplicate PASS, not binary shard rewrite, and not overall baseline completion.",
    }
