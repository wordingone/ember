#!/usr/bin/env python3
"""Materialize cumulative C1 near-duplicate exclusions v5 from band-48 partial adjudication remediation."""
...
BASE_MANIFEST = "fragments/c1-near-duplicate-cumulative-exclusions-v4-2026-06-30.jsonl"
BASE_RECEIPT = "receipts/4090-near-duplicate-cumulative-exclusion-manifest-v4-2026-06-30.json"
REMEDIATION_RECEIPT = "receipts/4090-cumulative-filtered-lsh-candidate-index-v4-band48-adjudication-partial25-remediation-2026-07-01.json"
OUT_MANIFEST = "fragments/c1-near-duplicate-cumulative-exclusions-v5-2026-07-01.jsonl"
VERDICT = "C1_NEAR_DUPLICATE_CUMULATIVE_EXCLUSION_MANIFEST_V5_READY_NOT_COMPLETION"
...
def main() -> int:
    ...
    if remediation.get("existing_cumulative_manifest_overlap_count") != 0:
        failures.append({"code": "remediation_overlap_not_zero", "actual": remediation.get("existing_cumulative_manifest_overlap_count")})
    ...
    for row in remediation.get("exclusions", []):
        cumulative.append({
            ...
            "shard": str(row["shard"]),
            "token_len": int(row["token_len"]),
            "source": "lsh_band48_partial25_adjudication_remediation",
    ...
    result = {
        "kind": "single_4090_c1_near_duplicate_cumulative_exclusion_manifest_v5",
        ...
        "completion_limit": "This materializes cumulative discovered-cluster exclusions v5 from partial band-48 LSH adjudication remediation only. It is not a full band-48 remediation, not an all-pairs near-duplicate PASS, not eval-contamination evidence, and not overall baseline completion.",
    }
