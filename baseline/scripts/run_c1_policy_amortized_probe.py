#!/usr/bin/env python3
"""Run a C1 policy-amortized streamed probe with nvidia-smi power sampling.
...
POWER_RECEIPT = "receipts/4090-policy-amortized-256-window-power-2026-06-30.json"
PROBE_RECEIPT = "receipts/4090-policy-amortized-256-window-probe-pretraining-equivalent.json"
...
def main() -> int:
    ...
    parser.add_argument("--token-offset", type=int, default=8_388_608)
    parser.add_argument("--token-stride", type=int, default=32_768)
    parser.add_argument("--steps", type=int, default=256)
    parser.add_argument("--checkpoint-interval-steps", type=int, default=128)
    ...
    receipt = {
        ...
        "kind": "single_4090_c1_policy_amortized_streamed_power_probe",
        "verdict": "C1_POLICY_AMORTIZED_256_WINDOW_READY_NOT_COMPLETION" if verdict else "C1_POLICY_AMORTIZED_256_WINDOW_INVALID",
        ...
        "completion_limit": "This is bounded nvidia-smi power sampling around a 256-window streamed real-token full-stack C1 train/checkpoint/eval/recovery policy probe only: not days-scale long-run training, not full-run energy accounting for data prep/eval/recovery/packaging, and not overall baseline completion.",
    }
    ...
    return 0 if receipt["verdict"] == "C1_POLICY_AMORTIZED_256_WINDOW_READY_NOT_COMPLETION" else 1
