#!/usr/bin/env python3
"""Run a C1 128-window streamed throughput probe with nvidia-smi power sampling.

The child probe receives local input paths, but this wrapper only records
repo-relative receipt paths and sanitized summary metrics.
"""
...
POWER_RECEIPT = "receipts/4090-power-sampled-128-window-throughput-2026-06-30.json"
...
def main() -> int:
    ...
    parser.add_argument("--token-offset", type=int, default=3_145_728)
    ...
    verdict = EXPECTED_PROBE_VERDICT == probe.get("verdict") and proc.returncode == 0
    receipt = {
        "created_at_utc": created_at,
        "kind": "single_4090_c1_power_sampled_streamed_throughput_probe",
        "verdict": "C1_POWER_SAMPLED_128_WINDOW_THROUGHPUT_READY_NOT_COMPLETION" if verdict else "C1_POWER_SAMPLED_128_WINDOW_THROUGHPUT_INVALID",
        ...
        "probe_receipt": {
            ...
            "includes_dataloader_timing": probe.get("includes_dataloader_timing"),
        },
        ...
        "completion_limit": "This is bounded nvidia-smi power sampling around a 128-window streamed real-token full-stack C1 probe only: not days-scale long-run training, not full-run energy accounting for data prep/eval/recovery/packaging, and not overall baseline completion.",
    }
    ...
    return 0 if receipt["verdict"] == "C1_POWER_SAMPLED_128_WINDOW_THROUGHPUT_READY_NOT_COMPLETION" else 1
