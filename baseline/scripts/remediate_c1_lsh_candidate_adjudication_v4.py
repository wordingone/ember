#!/usr/bin/env python3
"""Build deterministic remediation from v4 LSH candidate-index adjudication crossings."""
...
CUMULATIVE_EXCLUSIONS_V4 = "fragments/c1-near-duplicate-cumulative-exclusions-v4-2026-06-30.jsonl"
...
def main() -> int:
    ...
    existing_keys = read_exclusion_keys(root / CUMULATIVE_EXCLUSIONS_V4)
        ...
        for row in ordered[1:]:
            annotated = {
                **row,
                ...
                "kept": False,
                "reason": "cumulative_filtered_lsh_band48_partial25_adjudication_near_duplicate_component",
            }
    ...
    result = {
        "kind": "single_4090_c1_lsh_candidate_adjudication_remediation_v4",
        "created_at_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "verdict": "C1_LSH_CANDIDATE_ADJUDICATION_V4_REMEDIATION_PACKET_READY_NOT_COMPLETION",
        "source_adjudication_receipt": adjudication_rel,
        "source_cumulative_exclusion_manifest": CUMULATIVE_EXCLUSIONS_V4,
        ...
        "existing_cumulative_manifest_overlap_count": len(overlap),
        "rule": "Within each connected component of above-threshold v4 band-48 partial candidate-index adjudication pairs, keep the lowest doc_index/doc_sha256 document and exclude the remaining documents before any follow-up C1 pass attempt.",
        "clusters": clusters,
        "exclusions": exclusions,
        "scope_limit": "This remediation packet covers only crossing pairs found in the named partial candidate-index adjudication receipt. It is not a full band-48 remediation, not a full 16-band remediation, not a C1 near-duplicate PASS, and not overall baseline completion.",
