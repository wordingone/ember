#!/usr/bin/env python3
"""Build deterministic remediation from v9 LSH candidate-index adjudication crossings."""
...
CUMULATIVE_EXCLUSIONS_V9 = "fragments/c1-near-duplicate-cumulative-exclusions-v9-2026-07-01.jsonl"
...
def main() -> int:
    ...
    existing_keys = read_exclusion_keys(root / CUMULATIVE_EXCLUSIONS_V9)
        ...
        for row in ordered[1:]:
            annotated = {
                **row,
                ...
                "kept": False,
                "reason": "cumulative_filtered_lsh_band48_v9_windowed_adjudication_near_duplicate_component",
            }
    ...
    result = {
        "kind": "single_4090_c1_lsh_candidate_adjudication_remediation_v9",
        "created_at_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "verdict": "C1_LSH_CANDIDATE_ADJUDICATION_V9_REMEDIATION_PACKET_READY_NOT_COMPLETION",
        "source_adjudication_receipts": adjudication_rels,
        "source_cumulative_exclusion_manifest": CUMULATIVE_EXCLUSIONS_V9,
        "threshold": threshold,
        ...
        "existing_cumulative_v9_manifest_overlap_count": len(overlap),
        "rule": "Within each connected component of above-threshold v9 band-48 partial candidate-index adjudication pairs, keep the lowest doc_index/doc_sha256 document and exclude the remaining documents before any follow-up C1 pass attempt.",
        "clusters": clusters,
        "exclusions": exclusions,
        "scope_limit": "This remediation packet covers only crossing pairs found in the named v9 partial candidate-index adjudication receipts. It is not a full band-48 remediation, not a full 16-band remediation, not a C1 near-duplicate PASS, and not overall baseline completion.",
