#!/usr/bin/env python3
"""Materialize cumulative C1 near-duplicate exclusions v7 from v6 band-48 partial adjudication remediation."""
...
BASE_MANIFEST = "fragments/c1-near-duplicate-cumulative-exclusions-v6-2026-07-01.jsonl"
BASE_RECEIPT = "receipts/4090-near-duplicate-cumulative-exclusion-manifest-v6-2026-07-01.json"
REMEDIATION_RECEIPT = "receipts/4090-cumulative-filtered-lsh-candidate-index-v6-band48-adjudication-partial25-remediation-2026-07-01.json"
OUT_MANIFEST = "fragments/c1-near-duplicate-cumulative-exclusions-v7-2026-07-01.jsonl"
VERDICT = "C1_NEAR_DUPLICATE_CUMULATIVE_EXCLUSION_MANIFEST_V7_READY_NOT_COMPLETION"
...
def main() -> int:
    ...
    if remediation.get("existing_cumulative_v6_manifest_overlap_count") != 0:
        failures.append({"code": "remediation_overlap_not_zero", "actual": remediation.get("existing_cumulative_v6_manifest_overlap_count")})
    ...
    for row in remediation.get("exclusions", []):
        cumulative.append({
            ...
            "shard": str(row["shard"]),
            "token_len": int(row["token_len"]),
            "source": "lsh_band48_v6_partial25_adjudication_remediation",
    ...
    result = {
        "kind": "single_4090_c1_near_duplicate_cumulative_exclusion_manifest_v7",
        "created_at_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "verdict": VERDICT if not failures else "C1_NEAR_DUPLICATE_CUMULATIVE_EXCLUSION_MANIFEST_V7_INVALID",
        ...
        "completion_limit": "This materializes cumulative discovered-cluster exclusions v7 from v6 partial band-48 LSH adjudication remediation only. It is not a full band-48 remediation, not an all-pairs near-duplicate PASS, not eval-contamination evidence, and not overall baseline completion.",
    }
