#!/usr/bin/env python3
"""Validate the cumulative v9 near-duplicate filtered-corpus view receipt.
...
RECEIPT = "receipts/4090-cumulative-filtered-corpus-view-v9-2026-07-01.json"
EXACT_DEDUPE_RECEIPT = "receipts/4090-exact-dedupe-scan-2026-06-30.json"
EXCLUSION_RECEIPT = "receipts/4090-near-duplicate-cumulative-exclusion-manifest-v9-2026-07-01.json"
EXCLUSION_MANIFEST = "fragments/c1-near-duplicate-cumulative-exclusions-v9-2026-07-01.jsonl"
EXPECTED_VERDICT = "C1_CUMULATIVE_FILTERED_CORPUS_VIEW_V9_READY_NOT_COMPLETION"
...
def main() -> int:
    ...
    if filtered.get("excluded_document_count") != 1766 or filtered.get("excluded_token_floor") != 3087096:
        failures.append({"code": "v9_exclusion_totals_do_not_match_required_receipt", "actual_docs": filtered.get("excluded_document_count"), "actual_tokens": filtered.get("excluded_token_floor"), "expected_docs": 1766, "expected_tokens": 3087096})
    if filtered.get("remaining_document_count") != 4234692 or filtered.get("remaining_content_token_floor") != 6974781662:
        failures.append({"code": "v9_remaining_totals_do_not_match_required_receipt", "actual_docs": filtered.get("remaining_document_count"), "actual_tokens": filtered.get("remaining_content_token_floor"), "expected_docs": 4234692, "expected_tokens": 6974781662})
    ...
    result = {
        ...
        "verdict": "C1_CUMULATIVE_FILTERED_CORPUS_VIEW_V9_VALIDATED" if not failures else "C1_CUMULATIVE_FILTERED_CORPUS_VIEW_V9_INVALID",
        ...
        "completion_limit": "This validates a cumulative v9 filtered-corpus view only. It is not an all-pairs near-duplicate PASS, not binary shard rewrite, and not overall baseline completion.",
    }
