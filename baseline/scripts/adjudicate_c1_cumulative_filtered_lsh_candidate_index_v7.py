#!/usr/bin/env python3
"""Exact-Jaccard adjudicate a v7-filtered LSH candidate index.
...
FILTERED_VIEW = "receipts/4090-cumulative-filtered-corpus-view-v7-2026-07-01.json"


VERDICT_CLEAN = "C1_CUMULATIVE_FILTERED_LSH_CANDIDATE_INDEX_V7_EXACT_ADJUDICATION_NO_CROSSINGS_NOT_COMPLETION"
VERDICT_CROSSINGS = "C1_CUMULATIVE_FILTERED_LSH_CANDIDATE_INDEX_V7_EXACT_ADJUDICATION_CROSSINGS_FOUND_NOT_COMPLETION"
VERDICT_INVALID = "C1_CUMULATIVE_FILTERED_LSH_CANDIDATE_INDEX_V7_EXACT_ADJUDICATION_INVALID"


def read_index_rows(path: Path, max_buckets: int | None = None) -> list[dict[str, Any]]:
...
def main() -> int:
    ...
    parser.add_argument("--max-buckets", type=int, help="Adjudicate only the first N index rows for smoke/progress receipts.")
    ...
    rows = read_index_rows(index_path, args.max_buckets)
    ...
    partial = args.max_buckets is not None and args.max_buckets < int(candidate_receipt.get("index_row_count", 0))
    if failures:
    ...
    result = {
        "kind": "single_4090_c1_cumulative_filtered_lsh_candidate_index_v7_exact_adjudication",
