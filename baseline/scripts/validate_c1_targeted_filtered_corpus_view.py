#!/usr/bin/env python3
"""Validate the targeted near-duplicate filtered-corpus view receipt.
...
RECEIPT = "receipts/4090-targeted-filtered-corpus-view-2026-06-30.json"
EXACT_DEDUPE_RECEIPT = "receipts/4090-exact-dedupe-scan-2026-06-30.json"
EXCLUSION_RECEIPT = "receipts/4090-near-duplicate-targeted-exclusion-manifest-2026-06-30.json"
EXCLUSION_MANIFEST = "fragments/c1-near-duplicate-targeted-exclusions-2026-06-30.jsonl"
EXPECTED_VERDICT = "C1_TARGETED_FILTERED_CORPUS_VIEW_READY_NOT_COMPLETION"
...
def main() -> int:
    ...
    if not receipt_path.exists():
        failures.append({"code": "filtered_corpus_view_receipt_missing", "path": RECEIPT})
    if receipt.get("verdict") != EXPECTED_VERDICT:
        failures.append({"code": "filtered_corpus_view_bad_verdict", "actual": receipt.get("verdict")})
    ...
    result = {
        ...
        "verdict": "C1_TARGETED_FILTERED_CORPUS_VIEW_VALIDATED" if not failures else "C1_TARGETED_FILTERED_CORPUS_VIEW_INVALID",
        ...
        "completion_limit": "This validates a targeted filtered-corpus view only. It is not an all-pairs near-duplicate PASS, not binary shard rewrite, and not overall baseline completion.",
    }
