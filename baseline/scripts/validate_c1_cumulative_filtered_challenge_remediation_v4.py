#!/usr/bin/env python3
"""Validate deterministic v4 remediation from the cumulative-filtered v3 C1 challenge."""
...
RECEIPT = "receipts/4090-cumulative-filtered-challenge-remediation-v4-2026-06-30.json"
SOURCE_RECEIPT = "receipts/4090-cumulative-filtered-near-duplicate-sample-v3-2026-06-30.json"
...
def main() -> int:
    ...
    for row in exclusions:
        ...
        if row.get("kept") is not False or row.get("reason") != "cumulative_filtered_v3_challenge_near_duplicate_component":
    ...
    if "v4 remediation packet generated from cumulative-filtered v3 challenge-sample crossings only" not in str(receipt.get("scope_limit", "")):
    ...
    result = {
        ...
        "verdict": "C1_CUMULATIVE_FILTERED_CHALLENGE_REMEDIATION_V4_VALIDATED" if not failures else "C1_CUMULATIVE_FILTERED_CHALLENGE_REMEDIATION_V4_INVALID",
        ...
        "completion_limit": "This validates deterministic v4 remediation generated from cumulative-filtered v3 challenge-sample crossings only. It is not full-corpus near-duplicate remediation, not a C1 hygiene pass, and not overall baseline completion.",
    }
