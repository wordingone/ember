#!/usr/bin/env python3
"""Materialize cumulative C1 near-duplicate exclusions v2."""
...
TARGETED_MANIFEST = "fragments/c1-near-duplicate-targeted-exclusions-2026-06-30.jsonl"
TARGETED_RECEIPT = "receipts/4090-near-duplicate-targeted-exclusion-manifest-2026-06-30.json"
CHALLENGE_RECEIPT = "receipts/4090-targeted-filtered-challenge-remediation-2026-06-30.json"
CHALLENGE_VALIDATION = "receipts/4090-targeted-filtered-challenge-remediation-validation-2026-06-30.json"
OUT_MANIFEST = "fragments/c1-near-duplicate-cumulative-exclusions-v2-2026-06-30.jsonl"
VERDICT = "C1_NEAR_DUPLICATE_CUMULATIVE_EXCLUSION_MANIFEST_V2_READY"
...
def main() -> int:
    ...
    if challenge_validation.get("verdict") != "C1_TARGETED_FILTERED_CHALLENGE_REMEDIATION_VALIDATED":
    ...
    for row in targeted_rows:
        cumulative.append({
            ...
            "source": "targeted_cluster_expansion",
    ...
    for row in challenge_receipt.get("exclusions", []):
        cumulative.append({
            ...
            "shard": str(row["shard"]),
            "token_len": int(row["token_len"]),
            "source": "targeted_filtered_challenge_remediation",
    ...
    result = {
        "kind": "single_4090_c1_near_duplicate_cumulative_exclusion_manifest_v2",
        "created_at_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "verdict": VERDICT if not failures else "C1_NEAR_DUPLICATE_CUMULATIVE_EXCLUSION_MANIFEST_V2_INVALID",
        ...
        "completion_limit": "This materializes cumulative discovered-cluster exclusions only. It is not an all-pairs near-duplicate PASS, not a full-corpus MinHash proof, not eval-contamination evidence, and not overall baseline completion.",
    }
