#!/usr/bin/env python3
"""Run a C1 near-duplicate challenge sample after cumulative v2 exclusions."""
...
FILTERED_VIEW = "receipts/4090-cumulative-filtered-corpus-view-v2-2026-06-30.json"
EXCLUSION_MANIFEST = "fragments/c1-near-duplicate-cumulative-exclusions-v2-2026-06-30.jsonl"
...
def main() -> int:
    ...
    result = {
        "kind": "single_4090_c1_cumulative_filtered_near_duplicate_sample_v2",
        "created_at_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "verdict": "C1_CUMULATIVE_FILTERED_NEAR_DUPLICATE_SAMPLE_V2_NO_CROSSING_CANDIDATES" if not crossing and not failures else ("C1_CUMULATIVE_FILTERED_NEAR_DUPLICATE_SAMPLE_V2_CANDIDATES_FOUND" if not failures else "C1_CUMULATIVE_FILTERED_NEAR_DUPLICATE_SAMPLE_V2_INVALID"),
        ...
        "scope_limit": "Cumulative-filtered v2 deterministic challenge sample only. It is not an all-pairs near-duplicate PASS, not full-corpus MinHash proof, and cannot complete C1 data hygiene by itself.",
        "threshold": threshold,
        ...
        "sample_policy": "Select documents by stable blake2b document score after cumulative v2 exclusions and eligibility filtering.",
        "shards": shard_rows,
        "elapsed_seconds": round(time.time() - started, 3),
        "completion_limit": "This records cumulative-filtered v2 near-duplicate sample evidence only. It is not eval contamination evidence, not an all-pairs near-duplicate/MinHash PASS, not a long-run training receipt, and not overall baseline completion.",
    }
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(result, indent=2 if args.pretty else None, sort_keys=True) + "\n", encoding="utf-8", newline="\n")
    print(json.dumps(result, indent=2 if args.pretty else None, sort_keys=True))
    return 0 if result["verdict"] != "C1_CUMULATIVE_FILTERED_NEAR_DUPLICATE_SAMPLE_V2_INVALID" else 1
