#!/usr/bin/env python3
"""Validate cumulative C1 near-duplicate exclusion materialization v8."""
...
RECEIPT = "receipts/4090-near-duplicate-cumulative-exclusion-manifest-v8-2026-07-01.json"
BASE_VALIDATION_RECEIPT = "receipts/4090-near-duplicate-cumulative-exclusion-manifest-v7-validation-2026-07-01.json"
SOURCE_BASE_RECEIPT = "receipts/4090-near-duplicate-cumulative-exclusion-manifest-v7-2026-07-01.json"
SOURCE_REMEDIATION_RECEIPT = "receipts/4090-cumulative-filtered-lsh-candidate-index-v7-band48-adjudication-partial25-remediation-2026-07-01.json"
MANIFEST = "fragments/c1-near-duplicate-cumulative-exclusions-v8-2026-07-01.jsonl"
...
def main() -> int:
    ...
    if receipt.get("verdict") != "C1_NEAR_DUPLICATE_CUMULATIVE_EXCLUSION_MANIFEST_V8_READY_NOT_COMPLETION":
    ...
    if base_validation.get("verdict") != "C1_NEAR_DUPLICATE_CUMULATIVE_EXCLUSION_MANIFEST_V7_VALIDATED":
    ...
    if declared_count != base_count + remediation_count or declared_count != len(rows) or declared_count != 1747:
        failures.append({"code": "exclusion_count_mismatch", "declared": declared_count, "rows": len(rows), "base_plus_remediation": base_count + remediation_count, "expected": 1747})
    ...
    if receipt.get("exclusion_token_floor") != token_floor or token_floor != expected_floor or token_floor != 3058613:
        failures.append({"code": "token_floor_mismatch", "declared": receipt.get("exclusion_token_floor"), "rows": token_floor, "expected": expected_floor, "required": 3058613})
    ...
    if source_counts.get("cumulative_v7") != base_count:
        failures.append({"code": "base_source_count_mismatch", "source_counts": source_counts, "expected": base_count})
    if source_counts.get("lsh_band48_v7_partial25_adjudication_remediation") != remediation_count:
    ...
    result = {
        ...
        "verdict": "C1_NEAR_DUPLICATE_CUMULATIVE_EXCLUSION_MANIFEST_V8_VALIDATED" if not failures else "C1_NEAR_DUPLICATE_CUMULATIVE_EXCLUSION_MANIFEST_V8_INVALID",
        ...
        "completion_limit": "This validates cumulative discovered-cluster exclusions v8 only. It is not full band-48/full-corpus/all-pairs near-duplicate PASS evidence and not overall baseline completion.",
    }
