#!/usr/bin/env python3
"""Build deterministic remediation from v8 LSH candidate-index adjudication crossings."""
...
CUMULATIVE_EXCLUSIONS_V8 = "fragments/c1-near-duplicate-cumulative-exclusions-v8-2026-07-01.jsonl"
...
def main() -> int:
    ...
    existing_keys = read_exclusion_keys(root / CUMULATIVE_EXCLUSIONS_V8)
        ...
        for row in ordered[1:]:
            annotated = {
                **row,
                ...
                "kept": False,
                "reason": "cumulative_filtered_lsh_band48_v8_windowed_adjudication_near_duplicate_component",
            }
    ...
    result = {
        "kind": "single_4090_c1_lsh_candidate_adjudication_remediation_v8",
        "created_at_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "verdict": "C1_LSH_CANDIDATE_ADJUDICATION_V8_REMEDIATION_PACKET_READY_NOT_COMPLETION",
        "source_adjudication_receipts": adjudication_rels,
        "source_cumulative_exclusion_manifest": CUMULATIVE_EXCLUSIONS_V8,
        "threshold": threshold,
        ...
        "existing_cumulative_v8_manifest_overlap_count": len(overlap),
        "rule": "Within each connected component of above-threshold v8 band-48 partial candidate-index adjudication pairs, keep the lowest doc_index/doc_sha256 document and exclude the remaining documents before any follow-up C1 pass attempt.",
        "clusters": clusters,
        "exclusions": exclusions,
        "scope_limit": "This remediation packet covers only crossing pairs found in the named v8 partial candidate-index adjudication receipts. It is not a full band-48 remediation, not a full 16-band remediation, not a C1 near-duplicate PASS, and not overall baseline completion.",
