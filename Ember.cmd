...
powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "%~dp0scripts\prepare-ember-cockpit.ps1"
set "EMBER_EXIT=%ERRORLEVEL%"
if not "%EMBER_EXIT%"=="0" if not defined EMBER_LAUNCH_NONINTERACTIVE pause
exit /b %EMBER_EXIT%

:governed_cockpit
for /f "usebackq tokens=1,* delims==" %%A in (`powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "%~dp0scripts\prepare-ember-cockpit.ps1"`) do (
...
"%EMBER_LAB%" cockpit --root "%~dp0" --application "%EMBER_APPLICATION%" --source-commit "%EMBER_SOURCE_COMMIT%" --state-root "%EMBER_STATE_ROOT%"
set "EMBER_EXIT=%ERRORLEVEL%"
if not "%EMBER_EXIT%"=="0" (
  if not defined EMBER_LAUNCH_NONINTERACTIVE pause
  exit /b %EMBER_EXIT%
