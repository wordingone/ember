...
def main()->int:
 ...
 if not isinstance(score,dict) or score.get("criterion_id",expected)!=expected or score.get("criterion_result") not in ("PASSED","FAILED"):p.error("evaluator score artifact must provide the pinned criterion result")
 ...
 payload={"result":"PREFLIGHT_ONLY","admission":"NOT_ELIGIBLE","capability":a.capability,"subject_checkpoint_sha256":sha256(a.checkpoint_manifest),"benchmark_id":a.benchmark_id,"benchmark_version":a.benchmark_version,"split_sha256":sha256(a.split_artifact),"harness_sha256":sha256(a.harness_artifact),"protocol_sha256":sha256(a.protocol_artifact),"predictions_sha256":sha256(a.raw_predictions),"score_artifact_sha256":sha256(a.result_artifact),"sample_count":len(predictions),"metrics":metrics,"criterion_id":expected,"criterion_result":score["criterion_result"]}
 a.output.parent.mkdir(parents=True,exist_ok=True)
 with tempfile.NamedTemporaryFile("w",encoding="utf-8",dir=a.output.parent,prefix=a.output.name+".",suffix=".tmp",delete=False) as handle:
