#!/usr/bin/env python3
"""Clean-room REPL/UIUX component surface gate for Ember's avir-cli parity harness.

This gate does not import avir-cli UI code. It consumes the legal UI parity pass-2
        ...
        rows.append(
            {
                ...
                "source": cells[2],
                "rationale": cells[3],
                "has_brand_or_cloud_string": any(token in cells[3].lower() for token in ["brand string", "claude", "claude.ai", "anthropic", "desktop", "oauth", "subscription"]),
            }
        )
    return rows
