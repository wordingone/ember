...
Solve-rate definition (documented here, single source):
  ...
  where:
    verified_count = rows in receipts/ledger/episodes.jsonl with task == t
    failed_count   = rows in receipts/ledger/control_pool.jsonl with task == t
...
Frontier filter (docs/domains/governance/authority/STATE.md branch-registry row 9):
...
Launch interlock:
  ...
  an explicit --the lead-gate-token argument (any non-empty string) before it
...
ARMS = {
    ...
    "mtp": {
        ...
        "status": "concrete",
        "note": (
            "t2_mtp.main() reads from receipts/ledger/views/wcode-r1.jsonl (hardcoded path). "
            "Round-2 wrapper must pre-write receipts/ledger/views/wcode-r2.jsonl with the "
