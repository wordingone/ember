#!/usr/bin/env python3
"""Clean-room goal-mode bootstrap adapter for Ember's avir-cli resident harness.

This is the first executable adapter surface for the clean-room resident
harness precondition. It does not import avir-cli code. It implements the
...
DEFAULT_LAUNCH_SHIM = Path(r"tools\avir-goal-mode\avir.ps1")
...
def inspect_cleanroom_boundary(adapter_path: Path, launch_shim: Path, inventory: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "adapter_source": {
            ...
            "imports_avir_cli_code": False,
            "boundary": "clean-room Ember-side Python adapter; avir-cli source used only as inspected reference receipts",
        },
        "launch_shim": {
...
def select_blocker(goal_parse: dict[str, Any], inventory: dict[str, Any]) -> dict[str, Any]:
    selected = inventory.get("next_missing_precondition") or "avir_cli_goal_mode_parity_adapter"
    if selected == "avir_cli_goal_mode_parity_adapter" and goal_parse["status"] == "PASS":
        decision = "PASS_THIS_BOOTSTRAP_THEN_ADVANCE_TO_FULL_AVIR_CLI_PARITY"
    else:
    ...
    return {
        ...
        "selection_basis": [
            ...
            "non-killable clean-room avir-cli precondition",
        ],
    }
...
def build_receipt(
    *,
    ...
    next_command = (
        "python scripts\\ember_avir_cli_full_parity_harness_gate.py --out "
        "receipts\\ember-preloop-resident-gate\\avir-cli-full-parity-harness-gate-<timestamp>.json"
    )
    ...
    if inventory.get("inventory_verdict") != "AVIR_CLI_CLEANROOM_INVENTORY_BLOCKED":
    ...
    verdict = "AVIR_CLI_GOAL_MODE_PARITY_ADAPTER_PASS" if not blockers else "AVIR_CLI_GOAL_MODE_PARITY_ADAPTER_BLOCKED"
    return {
        "ticket": TICKET,
        "ts": ts(),
        ...
        "next_missing_precondition": "avir_cli_full_parity_harness_gate" if not blockers else "avir_cli_goal_mode_parity_adapter",
        ...
        "full_avir_cli_parity_requirement": {
            ...
            "next_gate": "AVIR_CLI_FULL_PARITY_HARNESS_GATE",
        },
        "n_rows": 6,
    }
...
def main() -> int:
    ...
    return 0 if receipt["verdict"] == "AVIR_CLI_GOAL_MODE_PARITY_ADAPTER_PASS" else 2
