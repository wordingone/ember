...
Authoritative condition (B:/M/avir/leo/state/ember-goal-leo.md, §4.3 C-MANIFEST):
...
Receipt/artifact inspected (REAL): B:/M/avir/leo/state/kai-converge/docs/ember-completeness.md
and the authoritative goal file's enumerated §4 conditions
(B:/M/avir/leo/state/ember-goal-leo.md), which ARE planned pieces of the organism.
...
Run (Git Bash):  wsl python3 \
  B:/M/avir/leo/state/ember-totality-build/test_c_manifest.py
"""
...
# --- locate kai-converge + goal file regardless of WSL vs Windows path conv --
...
# ember-goal-leo.md name; GOAL.md itself is tried too (its §4 may inline
...
def _goal_files_for(root: str) -> list[str]:
    return [
        ...
        os.path.join(root, "ember-goal-leo.md"),
    ]
...
def main() -> int:
    ...
    if nc_root is None:
        print(f"RED kai-converge root not found at any known path (tried {_NC_ROOTS})")
        return 0
