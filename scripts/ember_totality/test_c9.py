...
Authoritative condition (B:/M/avir/leo/state/ember-goal-leo.md, C9):
...
hardcodes the verdict. C9 is a STANDING PROCESS-INVARIANT (GOAL.md §4.0(9)),
...
Run (Git Bash):  MSYS_NO_PATHCONV=1 wsl python3 \
  /mnt/b/M/avir/leo/state/ember-totality-build/test_c9.py
"""
...
# --- locate kai-converge regardless of WSL vs Windows path conventions -------
...
def main() -> int:
    ...
    AUDIT-OK/AUDIT-INCIDENT/AUDIT-PENDING-EPOCH contract (GOAL.md §4.0(9))
    before printing."""
    root = _find_root()
    if root is None:
