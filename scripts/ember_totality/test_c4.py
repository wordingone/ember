...
Authoritative condition text (ember-goal-leo.md, line 104):
  C4 - Before/after.
...
far, under B:\M\avir\kai\state\... -- outside this tree entirely. Two new
...
CANDIDATE_DIR = os.path.join(KAI, "receipts", "ember-resident-training-candidate")
...
def main():
    ...
    # ---------------------------------------------------------------------------
    reach_ok, n_pairs, reach_detail = check_path_sha_pairs(cand, KAI)
    if not reach_ok:
        status("RED",
    ...
    if isinstance(trace_path, str) and trace_path:
        trace_on_disk = resolve_in_tree(trace_path, KAI)
