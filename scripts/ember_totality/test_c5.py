...
Authoritative condition text (ember-goal-leo.md, line 105):
  C5 - Positive delta.
...
Mapping to the fixed A/B/C/deleted contract (ember-goal-leo.md SS6):
...
def row_scores(row):
    """Pull the matched A/B/C scores from one per-task/per-slice row."""
...
def main():
    # ---- locate a REAL resident-training receipt carrying the A/B/C delta rows ----
    ...
    if pass_with_rows:
        ...
        status("RED", "resident-training receipts exist but NONE carry per-task/per-slice A/B/C rows -> invalid_no_rows")
    ...
    if malformed:
        status("RED", f"{receipt_name}: {malformed} per-task row(s) missing a/b/c_score -> rows not well-formed")
    if not c_vals:
        status("RED", f"{receipt_name}: per-task rows present but no parsable A/B/C scores")
    ...
    reach_ok, n_pairs, reach_detail = check_path_sha_pairs(cand_dict, KAI)
    if not reach_ok:
        status("RED",
               f"{receipt_name}: ARTIFACT REACHABILITY failed -- {reach_detail}. "
               f"the aggregate A/B/C delta cannot be trusted without an in-tree "
