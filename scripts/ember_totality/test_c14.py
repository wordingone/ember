...
C14 — RLM/iGRPO harness-native training organ (pre-loop gate; non-killable).
  R, conjunctive: unsupervised pretraining created the OWNED base (C-BASE;
    ...
    map; a row-by-row floor_contract_manifest; the fixed A/B/C/deleted contract
  ...
  Does NOT count: deterministic template selectors / scalar dictionaries /
    ...
    real-`B:/M/avir-cli/avir.exe` UI/UX/AX observation receipt
...
receipts/artifacts under kai-converge — never hardcoded.
...
Under WSL the B: drive is /mnt/b/ on this host.
"""
