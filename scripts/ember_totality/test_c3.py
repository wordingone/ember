...
Authoritative condition text (ember-goal-leo.md, line 103):
  C3 - Equal budget.
    R: A/B/C arms share declared+measured wall time, GPU/CPU, data access,
...
Gloss (task): equal budget = A/B/C arms share wall time, GPU/CPU, data access,
attempts, seed policy, verifier path.
Receipt hint (task): A/B/C/deleted contract receipts, per-arm budget rows.
...
WHERE C3'S EVIDENCE ACTUALLY LIVES (verified by inspection 2026-06-23):
  - The `ember-resident-training-gate/resident-training-gate-*.json` receipts carry the
    A/B/C/deleted contract and scores but have NO budget keys whatsoever.
  - The `ember-d3-native-loop/d3-native-loop-*.json` receipts carry a real
    ...
    `arm_contract` enumerating A/B/C/Deleted.
  So the d3-native-loop receipt is the canonical C3 budget artifact; this probe
...
This is a STATUS PROBE:
  ...
  - RED/GREEN is determined by REALLY inspecting the receipt under
    B:/M/avir/leo/state/kai-converge -- never hardcoded.
...
D3_LOOP_DIR = os.path.join(KAI, "receipts", "ember-d3-native-loop")
RESIDENT_GATE_DIR = os.path.join(KAI, "receipts", "ember-resident-training-gate")
...
def main():
    # ---- locate a REAL receipt carrying the A/B/C equal-budget block ----
    ...
    # A/B/C arm contract; that is the canonical C3 artifact. Never hardcode a path.
    chosen_path = None
    d = None
    ...
    # ---------------------------------------------------------------------------
    # (1) Positive CHK: per-arm budget rows present AND equal across A/B/C arms,
    ...
    if not arms_ok:
        status("RED",
               f"{receipt_name}: arm contract does not enumerate A/B/C "
    ...
    status("GREEN",
           f"C3 satisfied by {receipt_name}: A/B/C arm contract present and the "
