...
Authoritative condition text (ember-goal-leo.md, line 106):
...
RECEIPTS = os.path.join(KAI, "receipts")
...
def main():
    ...
    for path, d in eligible:
        ...
        if rv is not None:
            verified = (os.path.relpath(path, KAI), d, rv)
            break
    ...
    if verified is None:
        ...
        example = os.path.relpath(eligible[-1][0], KAI)
