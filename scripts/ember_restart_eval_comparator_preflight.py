...
def main() -> int:
    ...
    if comparator.get("subject_kind") != "open_comparator" or comparator.get("comparator_size_class") not in ("open_3b", "open_27b_or_31b") or not isinstance(comparator.get("comparator_revision"), str) or not re.fullmatch(r"[0-9a-f]{40}|[0-9a-f]{64}", comparator["comparator_revision"]):
