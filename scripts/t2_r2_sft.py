...
Launch interlock: requires --the lead-gate-token=<non-empty> on the command
...
def _require_gate_token():
    """Hard-refuse to proceed without --the lead-gate-token=<non-empty>."""
    ap = argparse.ArgumentParser(add_help=False)
    ap.add_argument("--the lead-gate-token", default="")
    args, _ = ap.parse_known_args()
    if not args.leo_gate_token.strip():
        print(
            "ERROR: t2_r2_sft.py requires --the lead-gate-token=<non-empty>. "
            "This is the launch-gate interlock for round-2 arms. "
            "Supply the token only when the lead's gate on the round-2 prereg "
