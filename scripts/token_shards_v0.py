...
`--emit` run is governed by the Step-1 chain (HOLD lifted 2026-06-11, the lead
14620). `--census` is the read-only diagnostic that sizes the text-borne
special-literal deviation (the lead 14628).
"""
import argparse
import hashlib
import json
...
# ---- frozen v0 shard contract (mirrors config/v0-pretrain-config.json + the
...
ENCODE_SEMANTICS = (
    ...
    "Doc-boundary separator id 0 is writer-inserted only. (the lead 14628: the "
...
def _encode_batch_factory(nc, tokfreeze, encode_fn, match_added_tokens=False):
    ...
    tokenizer_freeze.count_tokens uses — an agent-releasing, parallel across
...
def census_special_ids(nc, encode_fn=None, sources=None,
                       assembly_name=ASSEMBLY_RECEIPT,
                       tokfreeze_name=TOKENIZER_FREEZE_RECEIPT):
    """Read-only diagnostic (the lead 14628 — sizes the deviation, decides
    ...
    if a.census:
        ...
        payload = {
            ...
            "semantics": ("added-token literal matching ENABLED (legacy "
                          ...
                          "the matching-disabled re-derivation (the lead 14628, "
