...
free-margin assert, scripts/governor.py::preflight) plus a post-load
...
from ember_c14_contract_rig import _is_side_classifier  # noqa: E402  guard (e), read verbatim
...
class _OwnedCorePolicy(nn.Module):
    ...
    def __deepcopy__(self, memo: dict) -> "_OwnedCorePolicy":
        ...
        scripts/ember_c14_owned_run.py::_ActionBandPolicy.__deepcopy__, which
...
def _cuda_preflight() -> dict:
    ...
    rules (scripts/governor.py::preflight, the single canonical VRAM-
    ...
    def _factory() -> LoRAAdapter:
        ...
        if device == "cuda":
            ...
            assert allocated_delta < cap_bytes, (
                ...
                "scripts/governor.py's VRAM-fraction envelope, not this assert.)"
            )
...
def _run_smoke(device: str = "cpu") -> int:
    ...
    receipt = {
        ...
        "script": "scripts/ember_c14_owned_core.py",
        "verdict": verdict,
    }
    if is_cuda:
