...
Requirement 14 requires clean-room avir-cli resident harness parity, RLM,
...
""",
        ...
        _write(
            repo / "docs" / "ember-floor-contract.md",
            """
...
""",
        ...
        _write(avir_root / "avir.exe", "binary placeholder")
        ...
        receipt = gate.build_receipt(
            ...
            avir_root=avir_root,
            out=out,
            avir_command_definition=None,
        )

        assert receipt["verdict"] == "PRELOOP_RESIDENT_GATE_BLOCKED"
        assert receipt["gate_allows_loops"] is False
        assert receipt["preconditions"]["clean_room_avir_cli"]["status"] == "BLOCKED"
        assert receipt["preconditions"]["rlm"]["status"] == "BLOCKED"
        assert receipt["preconditions"]["igrpo"]["status"] == "BLOCKED"
        assert receipt["next_missing_precondition"] == "clean_room_avir_cli_resident_harness"
        assert "ember_avir_cli_cleanroom_inventory.py" in receipt["next_executable_command"]
