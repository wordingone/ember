...
Bench spec (the lead mail 15355, gate frozen):
...
# Gate thresholds (frozen, the lead mail 15355 — do not move)
GATE_TOK_S   = 25_463.0
GATE_NS5_TOL = 2e-7
...
def _run_bench() -> dict:
    ...
    muon_opt, adamw_opt, n_muon = fp45._build_opts(backbone, head, mtp_heads, "muon-batched", compiled_ns5=True, measure=False)
    ...
    def step():
        ...
        return t_muon + t_adamw, t_muon, t_adamw
    ...
    for i in range(TIMED):
        ...
        if (i + 1) % 5 == 0:
            print(f"[c04_ns5_bench]   step {i+1}/{TIMED} "
                  f"step={t_step*1000:.1f}ms muon={t_muon*1000:.1f}ms adamw={t_adamw*1000:.1f}ms", flush=True)
    ...
    print(f"[c04_ns5_bench] opt breakdown: "
          f"muon_wall={mean_muon*1000:.1f}ms adamw={mean_adamw*1000:.1f}ms", flush=True)
