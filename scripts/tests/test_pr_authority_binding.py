...
def test_wrong_goal_or_outcome_fails() -> None:
    ...
    assert "goal_id does not match docs/domains/governance/authority/GOAL.md" in validate_pr_body(body, GOAL, OUTCOME)
    body = f"goal_id: {GOAL}\nnext_executed_outcome: TBD\n"
    assert "next_executed_outcome does not match docs/domains/governance/authority/GOAL.md" in validate_pr_body(
        body, GOAL, OUTCOME
    )
...
def test_unknown_or_duplicate_child_workstream_fails() -> None:
    ...
    assert "workstream_id is not allowed by docs/domains/governance/authority/GOAL.md" in validate_pr_body(
