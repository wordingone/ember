#!/usr/bin/env python3
"""Clean-room legal/provenance boundary gate for Ember's avir-cli harness."""
...
class CleanroomLegalBoundary:
    ...
    def provenance_manifest(self) -> dict[str, dict[str, Any]]:
        ...
        manifest = {row.path: {"sha256": row.sha256, "bytes": row.bytes, "source_root": "avir_reference_root"} for row in files}
...
def run_probe(reference_root: Path | None = None) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="ember-legal-boundary-") as td:
        root = Path(td)
        ref = reference_root or (root / "avir-reference")
...
def build_receipt(*, repo: Path, avir_root: Path, communication_receipt: Path, out: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "verdict": verdict,
        "classification": "clean_room_reference_only_boundary_not_copied_avir_cli_or_prompt_only_provenance",
        "out_path": str(out),
...
def main() -> int:
    ...
    repo = Path(args.repo)
    out = Path(args.out)
    receipt = build_receipt(repo=repo, avir_root=Path(args.avir_root), communication_receipt=Path(args.communication_mailbox_computer_use_receipt), out=out)
