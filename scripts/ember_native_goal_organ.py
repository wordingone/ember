...
next blocker. It is not a Codex prompt trace and not the full avir-cli parity
gate by itself.
"""
...
DEFAULT_FULL_PARITY_RECEIPT = Path(r"receipts\ember-preloop-resident-gate\avir-cli-full-parity-harness-gate-20260621T190451Z.json")
...
def compile_bounded_action(native_receipt_path: Path) -> dict[str, Any]:
    command = (
        "python scripts\\ember_avir_cli_full_parity_harness_gate.py "
        f"--native-goal-organ-receipt {native_receipt_path} "
        "--out receipts\\ember-preloop-resident-gate\\avir-cli-full-parity-harness-gate-<timestamp>.json"
    )
    return {
        "status": "PASS",
...
def verify_organ(goal: dict[str, Any], full_parity: dict[str, Any], selection: dict[str, Any], action: dict[str, Any]) -> dict[str, Any]:
    checks = {
        ...
        "receipt_read_pass": full_parity.get("ticket") == "EMBER-AVIR-CLI-FULL-PARITY-HARNESS-GATE",
        ...
        "preserves_next_blocker": "ember_avir_cli_full_parity_harness_gate.py" in action["compiled_action"]["command"],
    }
    return {
