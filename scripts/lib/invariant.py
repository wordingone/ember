...
and verify them against the canonical docs/domains/governance/authority/INVARIANT.md file.
"""
...
# The canonical hash of docs/domains/governance/authority/INVARIANT.md as of genesis
INVARIANT_SHA256 = "08a0eb7418c09a8088be4658e10785107abbb7507fc2dbcdc789936aa54e02a6"


def _read_invariant_bytes(repo_root: str) -> bytes:
    """Read docs/domains/governance/authority/INVARIANT.md bytes from repo root.

    Args:
        repo_root: Absolute path to ember repo root

    Returns:
        Raw bytes of docs/domains/governance/authority/INVARIANT.md

    Raises:
        FileNotFoundError: If docs/domains/governance/authority/INVARIANT.md does not exist
    """
    invariant_path = Path(repo_root) / "docs/domains/governance/authority/INVARIANT.md"
    if not invariant_path.exists():
        raise FileNotFoundError(f"docs/domains/governance/authority/INVARIANT.md not found at {invariant_path}")
...
def verify(repo_root: str) -> str:
    """Verify that docs/domains/governance/authority/INVARIANT.md in repo_root matches the canonical hash.
    ...
    Raises:
        FileNotFoundError: If docs/domains/governance/authority/INVARIANT.md does not exist
        ValueError: If docs/domains/governance/authority/INVARIANT.md hash does not match INVARIANT_SHA256
    """
    ...
    if computed_hash != INVARIANT_SHA256:
        raise ValueError(
            f"docs/domains/governance/authority/INVARIANT.md hash mismatch: expected {INVARIANT_SHA256}, "
            f"got {computed_hash}"
        )
...
def stamp(artifact: Dict[str, Any], repo_root: str = None) -> Dict[str, Any]:
    ...
    Reads docs/domains/governance/authority/INVARIANT.md from repo root (or cwd if not specified),
...
def _selftest():
    ...
    1. The docs/domains/governance/authority/INVARIANT.md file exists and has the correct hash
    ...
    # Test 1: Verify current docs/domains/governance/authority/INVARIANT.md matches canonical hash
    try:
        cwd = os.getcwd()
        computed = verify(cwd)
        print("[PASS] Test 1: docs/domains/governance/authority/INVARIANT.md hash verified")
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            # tmpdir has no docs/domains/governance/authority/INVARIANT.md
            verify(tmpdir)
            print("[FAIL] Test 4: Expected FileNotFoundError")
            raise AssertionError("Should have raised FileNotFoundError for missing docs/domains/governance/authority/INVARIANT.md")
    except FileNotFoundError:
        print("[PASS] Test 4: Correctly raises FileNotFoundError for missing docs/domains/governance/authority/INVARIANT.md")
    except Exception as e:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            # Create a fake docs/domains/governance/authority/INVARIANT.md with different content
            fake_path = Path(tmpdir) / "docs/domains/governance/authority/INVARIANT.md"
