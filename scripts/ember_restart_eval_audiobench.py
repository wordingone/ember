...
def main()->int:
 p=argparse.ArgumentParser();p.add_argument("--run-artifact",required=True,type=Path);p.add_argument("--raw-predictions",required=True,type=Path);p.add_argument("--score-output",required=True,type=Path);a=p.parse_args()
 ...
 atomic(a.raw_predictions,mixtures)
 atomic(a.score_output,{"criterion_id":"ember-3b-audio-capability-v1","criterion_result":"FAILED","metrics":{"weighted_fpr":float(headline["weighted_fpr"]),"weighted_recall":float(headline["weighted_recall"])},"sample_count":len(mixtures),"upstream":"pinned local AudioBench run artifact"});return 0
