#!/usr/bin/env python3
"""Selftest for the clean-room avir-cli full parity harness gate."""
...
def main() -> int:
    import ember_avir_cli_full_parity_harness_gate as gate

    with tempfile.TemporaryDirectory(prefix="ember-avir-full-parity-") as td:
        root = Path(td)
        repo = root / "repo"
        avir_root = root / "avir-cli"
...
- **Current blocker:** full avir-cli parity gate.
- **Current executable command:** implement and run
  `python scripts\\ember_avir_cli_full_parity_harness_gate.py --out receipts\\ember-preloop-resident-gate\\avir-cli-full-parity-harness-gate-<timestamp>.json`.
- **Required receipt:** `AVIR_CLI_FULL_PARITY_HARNESS_GATE_PASS` or BLOCKED.
...
- **Next blocker if fail:** continue exact missing avir-cli full-parity surface.
""",
        )
        _write(
            inventory,
            json.dumps(
                {
                    ...
                    "launch_surface": {
                        ...
                        "avir_root_exists": True,
                        "avir_exe": {"path": str(avir_root / "avir.exe"), "exists": True},
                        "avir_ps1": {"path": str(avir_root / "avir.ps1"), "exists": False},
                    },
        ...
        _write(
            ...
            json.dumps(
                {
                    "ticket": "EMBER-AVIR-CLI-GOAL-MODE-PARITY-ADAPTER",
                    "ts": "20260621T000001Z",
                    "sha_convention": gate.SHA_CONVENTION,
                    "verdict": "AVIR_CLI_GOAL_MODE_PARITY_ADAPTER_PASS",
                    "does_not_satisfy_full_avir_cli_parity": True,
        ...
        assert receipt["verdict"] == "AVIR_CLI_FULL_PARITY_HARNESS_GATE_BLOCKED"
        ...
        assert any(row["source_root"] == "ember_repo" and row["path"] == "GOAL.md" and row["exists"] for row in native_refs)
        assert receipt["bootstrap_receipt"]["does_not_satisfy_full_avir_cli_parity"] is True
        ...
        assert receipt["next_executable_command"].startswith("python scripts\\ember_avir_cli_full_parity_harness_gate.py")
        ...
        _write(
            ...
            json.dumps(
                {
                    "ticket": "EMBER-AVIR-CLI-UIUX-REPL-COMPONENTS-GATE",
                    "ts": "20260621T000004Z",
                    "sha_convention": gate.SHA_CONVENTION,
        ...
        _write(
            ...
            json.dumps(
                {
                    "ticket": "EMBER-AVIR-CLI-BACKEND-COORDINATOR-AGENTS-GATE",
                    "ts": "20260621T000005Z",
                    "sha_convention": gate.SHA_CONVENTION,
            ...
            json.dumps(
                {
                    ...
                    "launch_packaging": {
                        "bin_name": "avir",
                        "launcher_path": "tools/avir-launch/avir.ps1",
                    ...
                    "deletion_ablation": {
                        ...
                        "stale_avir_ps1_pointer_insufficient": True,
        ...
        _write(
            ...
            json.dumps(
                {
                    "ticket": "EMBER-AVIR-CLI-HOOK-RUNNER-GATE",
                    "ts": "20260621T000008Z",
                    "sha_convention": gate.SHA_CONVENTION,
        ...
        _write(
            ...
            json.dumps(
                {
                    "ticket": "EMBER-AVIR-CLI-TOOL-DISPATCH-PERMISSIONS-GATE",
                    "ts": "20260621T000009Z",
                    "sha_convention": gate.SHA_CONVENTION,
        ...
        _write(
            ...
            json.dumps(
                {
                    "ticket": "EMBER-AVIR-CLI-STATE-PERSISTENCE-GATE",
                    "ts": "20260621T000010Z",
                    "sha_convention": gate.SHA_CONVENTION,
        ...
        _write(
            ...
            json.dumps(
                {
                    "ticket": "EMBER-AVIR-CLI-ROLLBACK-REWIND-GATE",
                    "ts": "20260621T000012Z",
                    "sha_convention": gate.SHA_CONVENTION,
        ...
        _write(
            ...
            json.dumps(
                {
                    "ticket": "EMBER-AVIR-CLI-CLEANROOM-LEGAL-BOUNDARY-GATE",
                    "ts": "20260621T000014Z",
                    "sha_convention": gate.SHA_CONVENTION,
                    ...
                    "deletion_ablation": {
                        ...
                        "copied_avir_cli_implementation_insufficient": True,
        ...
        assert legal_advanced["next_missing_precondition"] == "real_avir_uiux_ax_observation"
        assert legal_advanced["verdict"] == "AVIR_CLI_FULL_PARITY_HARNESS_GATE_BLOCKED"
        assert legal_advanced["classification"] == "FULL_PARITY_GATE_BLOCKED_NOT_HEADLESS_PASS"
        assert "real_avir_uiux_ax_observation_missing" in legal_advanced["blocked_reasons"]
        assert legal_advanced["real_avir_uiux_ax_observation_receipt"]["exists"] is False

        real_observation = receipt_dir / "real-avir-observation.json"
        _write(
            real_observation,
            json.dumps(
                {
                    ...
                    "classification": "REAL_AVIR_LIVED_SURFACE_OBSERVED",
        ...
        real_observed_advanced = gate.build_receipt(
            ...
            communication_mailbox_computer_use_receipt=communication_surface,
            cleanroom_legal_boundary_receipt=legal_surface,
            real_avir_observation_receipt=real_observation,
            out=out,
        )
        assert real_observed_advanced["next_missing_precondition"] == "rlm_igrpo_train_multimodal_adapter"
        assert real_observed_advanced["verdict"] == "AVIR_CLI_FULL_PARITY_HARNESS_GATE_PASS"
        ...
        assert real_observed_advanced["real_avir_uiux_ax_observation_receipt"]["verdict"] == "REAL_AVIR_UIUX_AX_OBSERVATION_PASS"
    print("EMBER_AVIR_CLI_FULL_PARITY_HARNESS_GATE_SELFTEST_PASS")
    return 0
