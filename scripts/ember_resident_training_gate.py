#!/usr/bin/env python3
"""Resident-training gate for Ember's non-killable RLM/iGRPO + avir-cli precondition.
...
def inspect_clean_room_harness(repo: Path, full_parity_receipt: Path | None = None) -> tuple[dict[str, Any], list[str]]:
    ...
    if full_parity_receipt is None or not full_parity_receipt.exists():
        ...
        real_observation = full_parity.get("real_avir_uiux_ax_observation_receipt") if isinstance(full_parity.get("real_avir_uiux_ax_observation_receipt"), dict) else {}
        full_parity_summary.update(
            {
                ...
                "real_avir_uiux_ax_observation_receipt": real_observation,
            }
        )
        if full_parity.get("ticket") != "EMBER-AVIR-CLI-FULL-PARITY-HARNESS-GATE":
            errors.append("clean_room_harness.full_parity_receipt_wrong_ticket")
        if full_parity.get("verdict") != "AVIR_CLI_FULL_PARITY_HARNESS_GATE_PASS":
        ...
        if real_observation.get("verdict") != "REAL_AVIR_UIUX_AX_OBSERVATION_PASS":
            errors.append("clean_room_harness.real_avir_uiux_ax_observation_missing_or_not_pass")
        for key in ["observed_real_avir_binary", "observed_real_tui", "observed_agent_loop", "observed_uiux_ax", "resource_governed"]:
            if real_observation.get(key) is not True:
                errors.append(f"clean_room_harness.{key}_not_true")
    return {
        "status": "FULL_AVIR_CLI_PARITY_PASS" if not errors else "INCOMPLETE",
        "files": files,
        "nck_e2e_proof": e2e_summary,
        "full_avir_cli_parity_receipt": full_parity_summary,
        "note": "Harness evidence is not a learned RLM/iGRPO resident-training organ by itself; full avir-cli parity is a required substrate for that organ.",
    }, errors
...
def build_gate_receipt(
    ...
    if harness_errors:
        ...
        if any("real_avir_uiux_ax_observation" in e for e in harness_errors):
            invalid_codes.append("real_avir_uiux_ax_observation_missing")
