...
import ember_avir_cli_backend_coordinator_agents_gate as gate
...
| `SendMessageTool/SendMessageTool` | `identical` | tools/SendMessageTool/SendMessageTool.ts:589 | Remote Claude cloud bridge. |
...
| `tasks/RemoteAgentTask/RemoteAgentTask` | `identical` | tasks/RemoteAgentTask/RemoteAgentTask.tsx:149 | Claude.ai remote. |
| `components/CoordinatorAgentStatus` | `derivative` | components/CoordinatorAgentStatus.tsx:34 | Task panel. |
| `skills/bundled/scheduleRemoteAgents` | `identical` | skills/bundled/scheduleRemoteAgents.ts:176 | Anthropic cloud scheduler. |
| `skills/bundled/claudeApi` | `identical` | skills/bundled/claudeApi.ts:184 | Claude API skill. |
| `skills/bundled/claudeApiContent` | `identical` | skills/bundled/claudeApiContent.ts:38 | Claude model docs. |
""",
        )
        _write(uiux, json.dumps({"ticket": "EMBER-AVIR-CLI-UIUX-REPL-COMPONENTS-GATE", "verdict": "UIUX_REPL_COMPONENTS_GATE_PASS"}))
    ...
    print("EMBER_AVIR_CLI_BACKEND_COORDINATOR_AGENTS_GATE_SELFTEST_PASS")
    return 0
