...
def verify(census_path: Path, dag_path: Path, ledger_path: Path, expected_master: str | None = None):
    ...
    for row in rows:
        ...
        if row.get("issue_number") == 764:
            ...
            reject(True, errors, not any(x.get("pr_number") == 1511 and x.get("head") == "bd2fbe9661debb817c3fb3173f6f6c1a93573dc2" for x in prs if isinstance(x, dict)), "#764 is not bound to the current-native producer PR")
