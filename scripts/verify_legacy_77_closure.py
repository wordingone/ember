...
def verify(census_path: Path, dag_path: Path, ledger_path: Path, expected_master: str | None = None):
    ...
    for row in rows:
        ...
        if row.get("issue_number") == 764:
            ...
            reject(True, errors, not any(x.get("pr_number") == 1511 and x.get("head") == "738099a8268b1118e8a919e907185836df121257" for x in prs if isinstance(x, dict)), "#764 is not bound to the current-native producer PR")
