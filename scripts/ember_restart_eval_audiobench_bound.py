...
def main():
 ...
 payload={'result':'PREFLIGHT_ONLY','claim_status':'NON_ADMISSIBLE_CLOSED_AUDIOBENCH_SCORER','criterion_id':'ember-3b-audio-capability-v1','criterion_result':'FAILED','metrics':metrics,'sample_count':len(rows),'checkpoint_manifest_sha256':envelope['checkpoint_manifest_sha256'],'model_config_sha256':envelope['model_config_sha256'],'benchmark_id':custody['benchmark_id'],'benchmark_version':custody['benchmark_version'],'split_sha256':envelope['benchmark']['split_sha256'],'suite':custody['suite'],'protocol_sha256':_protocol_sha256(custody),'predictions_sha256':hashlib.sha256(prediction_bytes).hexdigest(),'run_artifact_sha256':hashlib.sha256(run_bytes).hexdigest(),'frozen_audiobench_manifest_sha256':hashlib.sha256(custody_bytes).hexdigest(),'upstream':'closed AudioBench rows bound to canonical predictions'}
