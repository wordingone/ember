...
def ledger_records():
    ...
    path = f"{NC}/receipts/ledger/episodes.jsonl"
...
def acquire(source, out_root, byte_budget=None, shard_bytes=1_000_000_000):
    ...
    if spec["kind"] == "hf":
        ...
        ledger_path = f"{NC}/receipts/ledger/episodes.jsonl"
        url_pin = {
    ...
    manifest = {
        ...
        **({"deviation": spec["deviation"]} if "deviation" in spec else {}),
        ...
        "out_dir_wsl": (
            "/mnt/" + out_dir[0].lower() + out_dir.replace("\\", "/")[2:]
            if len(out_dir) >= 2 and out_dir[1] == ":"
            else out_dir.replace("\\", "/")
        ),
    }
