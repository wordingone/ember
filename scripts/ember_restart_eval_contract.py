...
FAMILIES = {
    "text", "reasoning", "mathematics", "coding", "sql", "browser_ui",
    "terminal", "image", "audio", "structured_tools", "efficiency", "retention",
    "deletion_ablation",
}
...
def validate(value: dict) -> list[str]:
    ...
    if not isinstance(target, dict) or target.get("kind") != "owned_checkpoint":
    ...
    for suite in suites:
        ...
        if any(not pattern.fullmatch(str(suite.get(field, ""))) for field, pattern in pins):
            errors.append("SUITE_PIN: " + str(suite.get("family", "unknown")))
    execution = value.get("execution")
    if not isinstance(execution, dict) or execution.get("runner") != "disk_budget_runner.py" or not execution.get("receipt_namespace"):
        errors.append("EXECUTION_RECEIPT_CONTRACT")
    return errors
...
def main() -> int:
    ...
    try:
        ...
        print("MANIFEST_READ: " + str(exc), file=sys.stderr)
        return 2
    errors = validate(value)
    if errors:
        print("\n".join(errors), file=sys.stderr)
        return 2
    print("EMBER_RESTART_EVAL_CONTRACT: VALID")
    return 0
