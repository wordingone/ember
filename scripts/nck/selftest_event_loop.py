...
def _base_config(tmp: str) -> dict:
    return {
        ...
        # in temp dirs where protected paths (docs/domains/governance/authority/GOAL.md, formalization-v0.md, etc.)
...
def case_g_mailsource_cold_no_flood(tmp: str) -> tuple[bool, str]:
    ...
    _make_maildb(db_path, [
        (1, "the lead", "ember", "old msg 1", "body1", "direct"),
        (2, "sage", "all", "broadcast 1", "bcast1", "broadcast"),
        (3, "the lead", "ember", "old msg 2", "body2", "direct"),
    ...
    conn.execute(
        "INSERT INTO messages VALUES (6, 'the lead', 'ember', 'new msg', 'new body', 'direct')"
    )
    conn.commit()
    conn.close()
...
def case_h_mailsource_timestamp_sig(tmp: str) -> tuple[bool, str]:
    ...
    _make_maildb(db_path, [
        (1, "the lead", "ember", "old", "oldbody", "direct"),
    ])
