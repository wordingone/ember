...
def _setup_stdout_echo(loop) -> None:
    """Print heartbeat lines and per-event dispatch lines to stdout (black-window fix)."""
    from datetime import datetime, timezone
    _tick_count: list[int] = [0]
    _event_count: list[int] = [0]

    def _on_heartbeat() -> None:
        _tick_count[0] += 1
        if _tick_count[0] % 30 == 0:
            ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
            print(f"[ember] heartbeat {ts} tick={_tick_count[0]} events={_event_count[0]}", flush=True)

    def _on_event(action, result) -> None:
        _event_count[0] += 1
        ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        print(f"[ember] event {ts} verb={action.verb}", flush=True)

    loop.heartbeat_hooks.append(_on_heartbeat)
    loop.post_action_hooks.append(_on_event)
