...
def _make_fixture_db(db_path: str, pre_rows: int = 1) -> int:
    ...
    for i in range(pre_rows):
        conn.execute(
            ...
            "VALUES (?, ?, ?, ?, ?)",
            ("the lead", "ember", f"pre-existing-{i}", f"body-{i}", "direct"),
        )
    conn.commit()
...
def stage_mail_consume(tmp_dir: str) -> dict:
    ...
    conn.execute(
        "INSERT INTO messages (from_id, to_id, subject, body, channel) "
        "VALUES (?, ?, ?, ?, ?)",
        ("the lead", "ember", "nck-e2e-proof-mail", "NCK e2e proof mail body", "direct"),
    )
    conn.commit()
    ...
    assert ev.payload.get("from") == "the lead", f"from mismatch: {ev.payload}"
