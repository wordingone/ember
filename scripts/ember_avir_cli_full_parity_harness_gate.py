#!/usr/bin/env python3
"""Clean-room full parity harness gate for Ember's avir-cli resident body.
...
DEFAULT_INVENTORY_RECEIPT = Path(r"receipts\ember-preloop-resident-gate\avir-cli-cleanroom-inventory-20260621T153049Z.json")
DEFAULT_BOOTSTRAP_RECEIPT = Path(r"receipts\ember-preloop-resident-gate\avir-cli-goal-mode-parity-adapter-20260621T155157Z.json")
DEFAULT_AVIR_ROOT = Path(r"<local-path>")
REAL_AVIR_OBSERVATION_PASS = "REAL_AVIR_UIUX_AX_OBSERVATION_PASS"
...
SURFACE_SPECS: list[dict[str, Any]] = [
    {
        ...
        "required_cleanroom_surface": "Ember-visible command registry with near-complete avir command behavior and goal-safe replacements for cloud-only/upstream-branded commands.",
    },
    {
    ...
    },
    {
        ...
        "reference_evidence": ["package.json", "avir.exe", "src/entrypoints/cli.tsx"],
        "required_cleanroom_surface": "Working avir/ember launch path, package/build contract, and Windows entrypoint that does not point at a missing avir.ps1.",
    },
    {
    ...
    },
    {
        ...
        "reference_evidence": ["src", "state", ".avir", "docs"],
    ...
    },
    {
        ...
        "reference_evidence": ["src/tools/SendMessageTool", "docs/avir/en/agent-teams.md", "docs/claude-code-upstream/en/sub-agents.md"],
    ...
    },
    {
        ...
        "reference_evidence": ["GOAL.md", "tools/avir-goal-mode/avir.ps1", "scripts/ember_avir_cli_goal_mode_parity_adapter.py"],
...
def reference_status(avir_root: Path, references: list[str], repo: Path | None = None) -> list[dict[str, Any]]:
    rows = []
    for rel in references:
        source_root = "avir_reference_root"
        path = avir_root / rel
...
def legal_receipt(inventory: dict[str, Any], avir_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "copying_policy": "reference/provenance inspection only; no avir-cli source import or inherited implementation copying",
    }


def inspect_real_avir_observation(receipt_path: Path | None) -> tuple[dict[str, Any], list[str]]:
    errors: list[str] = []
    ...
    summary = {
        ...
        "observed_real_avir_binary": data.get("observed_real_avir_binary"),
    ...
    if receipt_path is None or not receipt_path.exists():
        errors.append("real_avir_uiux_ax_observation_missing")
        return summary, errors
    if data.get("ticket") != "EMBER-REAL-AVIR-UIUX-AX-OBSERVATION":
        errors.append("real_avir_uiux_ax_observation_wrong_ticket")
    if data.get("verdict") != REAL_AVIR_OBSERVATION_PASS:
        errors.append("real_avir_uiux_ax_observation_not_pass")
    for key in [
        "observed_real_avir_binary",
        "observed_real_tui",
        "observed_agent_loop",
        "observed_uiux_ax",
        "resource_governed",
...
def build_surface_matrix(avir_root: Path, repo: Path) -> list[dict[str, Any]]:
    matrix = []
    for spec in SURFACE_SPECS:
        refs = reference_status(avir_root, spec["reference_evidence"], repo)
        matrix.append(
            {
...
def build_receipt(
    *,
    ...
    legal = legal_receipt(inventory, avir_root)
    real_avir_observation, real_avir_observation_errors = inspect_real_avir_observation(real_avir_observation_receipt)
    blocked_surfaces = [row["surface_id"] for row in matrix if row["status"] != "PASS"]

    blocked: list[str] = []
    if blocked_surfaces:
        blocked.append("full_cleanroom_parity_not_implemented")
        if bootstrap.get("does_not_satisfy_full_avir_cli_parity") is True:
            blocked.append("headless_bootstrap_only")
    if bootstrap.get("does_not_satisfy_full_avir_cli_parity") is not True:
    ...
    if inventory.get("ticket") != "EMBER-AVIR-CLI-CLEANROOM-INVENTORY":
        blocked.append("inventory_receipt_missing_or_wrong_ticket")
    if bootstrap.get("ticket") != "EMBER-AVIR-CLI-GOAL-MODE-PARITY-ADAPTER":
    ...
    blocked.extend(real_avir_observation_errors)
    ...
    verdict = "AVIR_CLI_FULL_PARITY_HARNESS_GATE_PASS" if gate_passed else "AVIR_CLI_FULL_PARITY_HARNESS_GATE_BLOCKED"
    ...
    next_parts = ["python scripts\\ember_avir_cli_full_parity_harness_gate.py"]
    ...
    next_parts.extend(["--out", "receipts\\ember-preloop-resident-gate\\avir-cli-full-parity-harness-gate-<timestamp>.json"])
    ...
    return {
        ...
        "bootstrap_receipt": {
            ...
            "does_not_satisfy_full_avir_cli_parity": bootstrap.get("does_not_satisfy_full_avir_cli_parity"),
        },
        ...
        "avir_root": {
            "path": str(avir_root),
            "exists": avir_root.exists(),
            "package_json_sha256": sha256(avir_root / "package.json") if (avir_root / "package.json").exists() else None,
        },
        "cleanroom_legal_boundary": legal,
        "cleanroom_legal_boundary_receipt": cleanroom_legal_boundary_evidence,
        "real_avir_uiux_ax_observation_receipt": real_avir_observation,
        "surface_matrix": matrix,
        "delete_ablate_required": delete_ablate,
        "blocked_reasons": blocked,
        "next_missing_precondition": blocked_surfaces[0] if blocked_surfaces else "real_avir_uiux_ax_observation" if real_avir_observation_errors else "rlm_igrpo_train_multimodal_adapter",
...
def main() -> int:
    ...
    parser.add_argument("--real-avir-observation-receipt")
    ...
    real_avir_observation = Path(args.real_avir_observation_receipt) if args.real_avir_observation_receipt else None
    if real_avir_observation is not None and not real_avir_observation.is_absolute():
        real_avir_observation = repo / real_avir_observation
    out = Path(args.out)
    receipt = build_receipt(
        ...
        real_avir_observation_receipt=real_avir_observation,
        out=out,
    )
    write_receipt(out, receipt)
    print(json.dumps(receipt, indent=2))
    return 0 if receipt["verdict"] == "AVIR_CLI_FULL_PARITY_HARNESS_GATE_PASS" else 2
