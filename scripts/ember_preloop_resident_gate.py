...
def inspect_goal(repo: Path) -> dict[str, Any]:
    ...
    markers = {
        ...
        "clean_room_the predecessor CLI": "clean-room `the predecessor CLI`" in text or "clean-room the predecessor CLI" in text,
    }
    return {
        "path": str(path),
        "exists": path.exists(),
...
def inspect_reference_root(reference_root: Path, reference_command_definition: str | None) -> dict[str, Any]:
    exe = reference_root / "predecessor-cli.exe"
    ps1 = reference_root / "launch.ps1"
    parity_receipts = sorted(reference_root.glob("**/*cleanroom*receipt*.json")) if reference_root.exists() else []
    implementation_files = sorted((reference_root / "src").glob("**/*"))[:50] if (reference_root / "src").exists() else []
    ...
    blockers = []
    if not ps1.exists():
        blockers.append("launch_ps1_surface_missing_or_command_points_to_missing_script")
    if not parity_receipts:
        blockers.append("clean_room_the predecessor CLI_parity_receipt_missing")
    if not implementation_files:
        blockers.append("the predecessor CLI_source_inventory_missing")
    return {
        ...
        "launch_ps1": {"path": str(ps1), "exists": ps1.exists()},
        ...
        "blockers": blockers,
        "required_receipt": "clean-room the predecessor CLI resident harness parity receipt with /goal-mode-equivalent action loop, deletion-sensitive harness action channel, and no copyright taint",
    }
...
def build_receipt(
    *,
    ...
    reference_root: Path,
    out: Path,
    reference_command_definition: str | None = None,
) -> dict[str, Any]:
    ...
    ref_cli = inspect_reference_root(reference_root, reference_command_definition)
    ...
    preconditions = {
        "clean_room_the predecessor CLI": precondition_block(
            "BLOCKED",
            list(ref_cli["blockers"]) or ["clean_room_predecessor_cli_load_bearing_resident_evidence_missing"],
            ref_cli,
            "implement clean-room the predecessor CLI resident harness inventory/parity receipt",
        ),
    ...
    next_missing = "clean_room_the predecessor CLI_resident_harness"
    next_command = (
        "python scripts\\ember_gate_cleanroom_inventory.py --out "
        "receipts\\ember-preloop-resident-gate\\gate-cleanroom-inventory-<timestamp>.json"
    )
    blockers = (
        ["goal_authority_incomplete"] if goal["status"] != "PRESENT" else []
    ) + paper_errors + list(ref_cli["blockers"]) + rlm_blockers + igrpo_blockers
    ...
    return {
        ...
        "kill_ablate_test_required": "deleting any of clean-room the predecessor CLI resident harness, RLM organ, or iGRPO organ must block or degrade the gate",
