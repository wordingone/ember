...
NC_WIN = "<local-path>"
LEDGER = f"{NC_WIN}/receipts/ledger/episodes.jsonl"
CONTROL = f"{NC_WIN}/receipts/ledger/control_pool.jsonl"
...
def _canon_path(p):
    ...
    Converts both Windows-style '<local-path>' and WSL-style '<local-path>'
...
def classify(rec):
    ...
    Both Windows ('<local-path>') and WSL ('<local-path>') adapter paths
...
def _selftest():
    ...
    # AC1: WSL-style path (<local-path>) must also match ADAPTERS_DIR
    # Derive WSL equivalent: <local-path> -> <local-path>
