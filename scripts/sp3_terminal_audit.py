...
ROWS = (
    ...
    {"id": 2, "condition": "Generates verified experience",
     "requires": (("receipts/v-soundness-probe-*.json", 1),
                  ("receipts/ledger/views/grpo-r2-tasks.json", 1))},
...
def _tracked(nc):
    ...
    verdict (an agent 14631). Portability is enforced, not remembered."""
    import subprocess
...
def audit_row(row, nc=NC, tracked=None):
    ...
    for req in row["requires"]:
        ...
        if len(ok) < min_count:
            ...
            # supersedes an earlier one on the same surface (an agent 14541
...
def _selftest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        open(f"{td}/receipts/ledger/view.json", "w").write("{}")
        r_ok = audit_row({"id": 1, "condition": "c",
                          "requires": (("receipts/good-*.json", 1),
                                       ("receipts/ledger/view.json", 1))}, nc=td)
    ...
    # untracked evidence can never satisfy a row (an agent 14631 class):
