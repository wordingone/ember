...
Both shortcuts target only stable installed files. The icon comes from the currently admitted Ember executable. The executable embeds the tracked `tools/ember-cli/src/assets/ember.ico`; `build-cockpit.ts` binds that exact asset through Bun's Windows metadata arguments.
