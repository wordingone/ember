...
| C-MANIFEST | S6 | Completeness manifest enumerates every planned piece | docs/contracts/ember-completeness.md rows cover every §4 goal condition (id+subgoal+AC+receipt+status); an absent piece is a gate violation (does-NOT-count) | scripts/ember_totality/receipts-totality/ember-totality-20260710T150632Z.json (C-MANIFEST row, evaluated by scripts/ember_totality/test_c_manifest.py) | DONE |
