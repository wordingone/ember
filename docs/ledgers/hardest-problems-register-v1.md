...
**Proof obligation + program:** `docs/domains/governance/archive/pre-restart/compute-ceiling-program-v1.md` —
...
**Status:** OPEN — program active, the engineer lane #1, first benches in governed
...
successes/100). an agent's standing hypothesis — verified signal per GPU-hour is
...
1. **Tonight 06-12:** production-path cell = true F (the engineer, <1h, GO'd
   15078); L9 completion b16/b8-nockpt-flash (<1h).
2. **06-13 AM:** fp-39 recalibration (the lead, mechanical on leg-1 receipts) +
   activation-model refit; density-A/B mix manifests frozen (the engineer proposes,
   the lead gates); density A/B = 4 cells ≈ 4 governed hours.
3. **06-13:** c04 design + harness build (flash/SDPA module, optimizer
   ≤15% wall, compile-resident, MTP/CE priced — ENG build, the least
   receipted leg: ~1 the engineer-day, the honest schedule risk) → #353 design
...
receipt. Owner: the lead, on the engineer's receipts.
