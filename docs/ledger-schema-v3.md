...
| `license_class`, `license_basis` | (eng #70) pre-stamping record — derivable by `fp6_provenance.classify` from `sampler`/`origin`; the sidecar view `receipts/ledger/views/license-class.jsonl` carries the backfilled mapping. New appends stamp at ingest. |
