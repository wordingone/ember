...
the engineer proposal; the lead gates before freeze. Linked from c04-token-budget-v1.md
...
Both arms tokenize from `<local-path>`
...
- **the engineer:** shard prep (arm B tokenization) + training harness + receipt emission
- **the lead:** approve this spec (gate before freeze), gate the verdict, update c04 budget doc at fp-39-recalibrated rates
- **Order:** fp-39 prod bench first → recalibrate budget table → then density A/B bench

---

## 9. the lead gate addenda (GATED-GO conditions, mail 15082, 2026-06-12)
