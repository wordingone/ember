...
|----|-------------|--------------------------------------|
| A1 | operators off | process snapshot pre/post: zero assistant-model sessions in operator trees interacting; operator mailbox outbound count for the window == 0 (sqlite, sender in operator set, injector identity excluded) |
