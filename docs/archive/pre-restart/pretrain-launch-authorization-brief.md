# C04 pretrain launch — authorization brief (maintainer-facing, pre-staged)
...
receipt + the maintainer authorization** — it is not a checkpoint to start silently. This
...
It is the rung the 8 gated the lead issues sit behind.
...
- **PROFILE B — clean bench short (< 25462.963)**: the **genuine tradeoff**, the
  ...
  - **Take AdamW** → clears §3 free at 0.919 d. Cost: the measured quality gap =
    ...
    an agent-flagged**: 0.141 nats over the 0.605 noise floor, and it collapses to
...
The clean no-sync production bench receipt (the engineer; GPU-serialized behind the
