...
end-to-end (not asserted): a synthetic the engineer-shaped fp44 receipt run through the
...
So the true critical path after fp44 lands is **the engineer dispatching the batched-NS5
bench** (PR #390 harness), not the fp44 verdict. the engineer has this queued.
...
- **Footprint wall broken** (the engineer `empty_cache` after warmup; hyp-A confirmed —
...
- **The escalation is HELD (break-the-wall) — one lever untried before the maintainer:**
...
1. the engineer compiles the optimizer step (+ per-shape-group bmm, not all-stack).
2. compiled+per-group step clears §3 → COMMIT_MUON_BATCHED, Muon kept, NO the maintainer
   escalation. gate-9 muon_batched authorization verified open (PR #405).
3. ONLY if it still can't clear §3 → torch≥2.7 is the genuine the maintainer tradeoff below
   (env-risk for Muon's thin/an agent-flagged quality edge vs AdamW free at 0.919d).
   The measured Muon number is now 7212-class, not 19223 — refresh the ESCALATE
   numbers from the engineer's compiled-step receipt before presenting to the maintainer.

## Verdict → action (verified composition)

| fp44 verdict | batched-NS5 state | pick output | action |
|---|---|---|---|
| any | absent, still expected | `PENDING` | wait for batched-NS5 (the engineer dispatches) |
...
the lead presents the **measured delta** (muon−adamw @2000) and the noise floor;
...
Verdict comes from running the scorer on the engineer's receipt — never from the live
training log (separation of concerns). `noise_floor_source` in the fp44 gate
output must read `derived` (it read the engineer's Phase-1 floor); a `default` there is
