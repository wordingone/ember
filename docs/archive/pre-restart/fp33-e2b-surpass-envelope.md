...
The user's actual ask, stated explicitly: the lead acts as the one who SOLVES an
...
kernel-level throughput research on sm89, owned by the lead's hands:
...
- **Selection discipline:** #61 (GPU bottleneck ledger, user via an agent 14633)
...
- Ownership: kernel research+engineering = the lead (research lane, by user
  direction); daemon ops, dispatch, profiling-run execution = the engineer.
...
## Execution legs (the engineer, sequenced; CPU/light until prereg freezes)
...
- **Autonomous-agent recipe research is now the demonstrated SOTA method:**
  Prime Intellect (May 2026) — assistant-model CLI + Codex agents ran ~10k experiments
