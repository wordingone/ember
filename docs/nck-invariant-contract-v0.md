...
*Provenance: synthesized 2026-06-10 from an 8-subsystem clean-room audit (workflow wf_c8be6869-6d1, 9 agents); reviewed and gated by the lead. v0 — invariants are behavioral contracts, not file maps; the internal detailed map carries the per-subsystem evidence.*
...
| In-process meta-search engine (`services/search/`: parallel DDG+Brave+Wikipedia, merger, cache) | **Exteroception** — the kernel's owned external-discovery organ; the upstream's WebSearch is a server-side tool (`web_search_20250305`) that does not exist for a local backend | **Keep** — already a clean-room client-side reimplementation; added 2026-06-10 (was missing from the 8-subsystem audit scope) |
| Client-side fetch + HTML→markdown (`WebFetchTool`: axios + turndown/domino) | Exteroception — owned acquisition of external content; fetched content is DATA, never instructions (injection boundary is a kernel invariant, not a UI nicety) | **Keep** — client-side already; axios replaceable with native fetch + redirect-policy port |
