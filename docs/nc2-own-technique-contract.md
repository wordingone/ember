...
**What:** the maintainer's quantization tooling (predecessor-CLI fragment; proven on this box for GGUF
...
this is the one place in this project where 1.58-bit is even reachable.
...
**Resolved by research receipt** (`docs/research/subq-term-resolution.md`): "SubQ" in
...
## 6. SDEK — adaptation-control kernel (owned, audited)

**What** (from the SDEK research handoff, 2026-06-08): local
...
receipts-as-claim-ledger. The SDEK v0 success criterion ("one temporary adapter becomes
...
**Deep-dive findings (2026-06-10, receipt: docs/research/gemma4-unified-architecture.md):**
...
## Survey verdicts → component dispositions (2026-06-10, receipt: docs/research/nc2-technique-survey-2026-06.md)
...
| 8 | Gemma-4 unified | deep-dive landed (docs/research/gemma4-unified-architecture.md): "unified" = Fuyu-style continuous soft tokens (48×48px patch → single matmul embedder, ~35M params), per-span bidirectional attention inside the causal decoder, 2D RoPE, vocab unchanged (~7 reserved delimiter IDs). Smallest working encoder-free prior art = 1.8B (Mono-InternVL); sub-1B UNVERIFIED | ADOPT — lock 4 v0 contracts (below); embedder/audio/QAT/MTP all retrofit-proven |
| 9 | DiffusionGemma / dLLM decode (USER-SOURCED 2026-06-10, released same day) | 26B MoE, 3.8B active, Apache 2.0, block-parallel diffusion decode: ~4× faster, >1K tok/s on H100, ~18GB VRAM (fits 4090), vLLM-native day one; output quality BELOW autoregressive Gemma 4 per Google's own framing | WATCHING — two distinct bets: (a) as SAMPLER/teacher-generator: feed math F = verified-episodes-per-GPU-hour is throughput-bound and V disposes of junk, so a 4×-throughput/lower-quality sampler is favorable iff its verify-rate floor stays nonzero — probe = w1-style MBPP floor run, DiffusionGemma vs Qwen-3B, same k/tasks/governor, decided on measured F (§7b admission symmetry; ~18GB pull needs no escalation, stays on-PC); (b) as NC2-own architecture component (diffusion head on owned core): premature — no sub-1B diffusion-LM evidence; revisit on NC2 entry. Revision trigger: any local probe receipt or a ≤1B dLLM publication |
