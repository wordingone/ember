...
the maintainer-via-an agent 14492):** every load-bearing choice below carries one of
...
| schedule/timeshare | eng-33 harness (#125) | MERGED (CPU selftest BIT_EXACT; dry-run continuity CLOSE at rtol=2.0; live/GPU resume-integrity RECEIPT required at the v0 run — an agent 14510) |
| corpus | eng-36 (#130) receipts: per-source license + sha + dedup + token counts | OPEN (the engineer, GO) |
