...
| Property | Value | Source / Note |
|----------|-------|---|
| **Pinned as** | avir-cli `turboquant` module (user's quantization tooling) | <local-path> (currently deferred; intake via symbol reference) |
...
| Property | Value | Source / Note |
|----------|-------|---|
| **Pinned as** | an agent's SDEK v0 kernel (per `an agent-index/sdek-research-handoff.md`, 2026-06-08) | https://github.com/anthropic-ai/ai-research-an agent (user private repo; internal reference). Summarized in contract §6. |
| **License** | Owned by an agent / Anthropic | SDEK is internal R&D; not a third-party dependency |
...
3. **turboquant status:** Confirm avir-cli post-NC2-entry timeline with the engineer/user. If critical path, accelerate intake.
...
**Draft authored:** 2026-06-10 (the lead / Claude Code subagent)  
