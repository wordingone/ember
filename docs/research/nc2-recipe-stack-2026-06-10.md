...
| 2 | Export/KV pipeline | predecessor-CLI `turboquant` (local, deferred intake) | maintainer-owned | local; unchanged from draft |
...
| 7 | SDEK kernel | local: `<local>/sdek-research-handoff.md` | internal | local handoff doc; a prior draft URL was not a real pin — **removed**. |
...
6. turboquant intake timing (predecessor-CLI deferred; escalate if critical path).
