...
Target: `<local-path>` (366 lines, FastAPI). Cross-founder infra — this spec goes to the lead BEFORE any edit (mail condition, 14476). Minimal surface: 5 hunks, eval/export paths only, ZERO train-path behavior change.
...
1. the lead signs off this spec (#105 condition).
...
- server.py lives in shared infra OUTSIDE this repo (<local-path>); the change is committed in that repo — this file + the verification receipt are the in-repo record.
