...
|----|-------------|--------------------------------------|
| A1 | founders off | process snapshot pre/post: zero claude.exe/codex sessions in founder trees (the lead/the engineer/an agent/an agent/an agent/sage/an agent) interacting; founder mailbox outbound count for the window == 0 (sqlite, sender in founder set, injector identity excluded) |
...
- an agent: adversarial pass over this spec BEFORE first run (attestation
