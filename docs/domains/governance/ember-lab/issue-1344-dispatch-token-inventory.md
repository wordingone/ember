...
| --- | --- | --- |
| `scripts/verify_ember01_completion.py` | assembles the EMBER-01 nine-condition completion receipt (reads/hashes, never launches) | `services/verify-watch.ts` (`child_process.spawn`, polled) |
...
| `src/ember/infrastructure/tools/ember-restart-3b/launch_packet.py` | "EMBER-01 cond7 launch-packet readiness runner (**CPU-only, no GPU allocation**)" (its own docstring) | `commands/train.ts` preflight (`runLaunchPacket`), spawned directly, **before** any certified-launch offer exists |
...
assigned anywhere under `src/ember/infrastructure/tools/ember-cli/src`), not a regression introduced
