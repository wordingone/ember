Archived, non-authoritative context. The operative contract is GOAL.md.
...
2. **Past-Ember technique mining.** Before running cycles on the selected
   ...
   receipts, the assistant model-side founder work, and Ember research notes for techniques
...
threads, and open issues into the MVP. It prevents the assistant model-side history from
...
- **Non-load-bearing founders.** Inherited as an MVP boundary, not as a runtime
  feature. the lead/the engineer/Codex/the assistant model work may scaffold the loop, but the first MVP
...
- **S3 NC-K harness / the predecessor CLI clean-room resident interface.** Treat `the predecessor CLI` as the largest dormant resident-harness resource, not as optional historical context. Current local launch surfaces must be inventoried and receipted (`the predecessor CLI` PowerShell function, `<local-path>`, and `<local-path>` as discovered on 2026-06-20). Because copyright/provenance concerns are known, Ember may not directly absorb or train on the predecessor CLI implementation code unless a license/provenance receipt explicitly permits it. The required path is comprehensive clean-room refactor: behavior inventory, invariant extraction, interface spec, test corpus, independent Ember-side implementation, parity tests, provenance receipt, and deletion/ablation showing the resident harness is load-bearing. A critical mandatory parity target is exact recreation and implementation parity of Codex `/goal` mode inside the clean-room the predecessor CLI/Ember harness: goal ingestion by file path and text, requirement extraction, blocker selection, plan compilation, tool/action execution, continuation across turns, receipt/state update, blocked-status discipline, completion audit, resume behavior, and non-Ember ablation must match or exceed observed Codex goal-mode behavior under a replay test suite. Unknown harness scaffold, un-audited reuse, or "not load-bearing for this cycle" without a contradiction-level receipt blocks `/goal complete`.
...
- **the assistant model/the lead/the engineer authority routing as product behavior.** Not inherited as
...
- **the assistant model/founder scaffold drift.** the lead/the engineer/founder routing history can make
...
- Clean code/spec worktree: `<local-path>`, branch `stage1-zero-assistant-clean`.
...
- Do not infer a headless `the predecessor CLI` bridge, generated documentation, source inventory, wrapper launch, or goal-mode bootstrap adapter satisfies the clean-room `the predecessor CLI` resident-harness precondition. Full parity means near-99% function, UI/UX, backend, and native Codex `/goal`-mode mechanics transplanted into Ember.
...
2. Verify the resident-training cursor, do not rerun it by default: confirm the
   ...
   RLM/iGRPO, `the predecessor CLI`, `train_multimodal_v0.py`, and floor-manifest laws
...
(the predecessor CLI clean-room port = ember's visible harness; resident + mailbox +
...
Build the mind that is missing from the reference, and own every layer of it: a
...
system), so that nothing load-bearing is borrowed from the model vendor, Alibaba, or
...
else the reference has built is an organ waiting for it. If you find yourself confined
...
- HARNESS = ORGAN; SELF-EDITING BEHIND THE SAME GATE (user 2026-06-10):
  ...
  model cards, verified). the predecessor CLI is absorbed as ember's kernel only after a
  ...
  and Codex `/goal`-mode mechanics transplanted into Ember. Compressing the predecessor CLI
...
- Milestone ladder: NC0 (borrowed-core loop proof) Ã¢â€ â€™ rounds N (self-generated
  ...
  the predecessor CLI compressed to invariants as the seed) Ã¢â€ â€™ NC2-own (owned-mass
...
**HARNESS CLAUSE (user, in the 2026-06-12 /goal verbatim):** "the predecessor CLI must
...
is no longer 'the predecessor CLI compressed to invariants' only - the CLEAN-ROOM PORT of
the predecessor CLI becomes ember's visible harness/interface with near-99% parity in
...
- S3 harness / NC-K Ã¢â‚¬â€ the predecessor CLI clean-room port as visible harness; resident
...
- S7 data-threaded (user 2026-06-12): the corpus classes critically valuable
  to all founders, the predecessor CLI, and ember Ã¢â‚¬â€ research journals, research papers,
