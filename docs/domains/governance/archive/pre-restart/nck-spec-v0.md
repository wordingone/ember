...
invariant gate. Seed = the predecessor CLI compressed to its invariants, not adopted
wholesale.
...
- Ember's editable tree: `kernel/` (its own code), `state/` (its working
  memory), `receipts/ledger/` (append-only episodes). Everything else is outside
  the write surface.
...
## Predecessor-CLI compression list (the seed, extract-not-adopt)
