# sp-5 — NC-K harness: predecessor-CLI clean-room port spec v0 (Closes #257)

Goal clause (per GOAL.md): "the predecessor CLI must be
...
- No predecessor-CLI source code enters the port tree — not copied, not adapted, not
  "referenced while writing." The port implementer works from THIS spec only.
- This spec was authored from the predecessor CLI's observable behavior and the goal's
  named invariant classes (process supervision, hooks, tool dispatch, state
  persistence) — it contains no predecessor-CLI code.
- Provenance receipt required at port PR time: tree-level attestation that no
  file derives from `<local>` (path-disjoint diff base + reviewer
  check; deterministic: grep for predecessor-CLI-unique identifiers = empty).
...
1. **Process supervision.** Child jobs (sampling, training, eval) are
   ...
   (the predecessor-CLI Job-Object lesson, generalized: every child runs inside a
...
- **Mailbox:** ember gets a founders.yaml identity (`ember`). The user and
  the lead mail it; it mails back. Signal-file wiring identical to founders.
  (infra/ change — cross-founder coordination, the engineer executes.)
- **Computer use:** ember's visible interface is a console session that
  ...
  and renders its output as text — so the user AND the lead can drive it exactly
