...
the engineer's eng-363 `equiv_pass` is **100-step** (losses → 0 by step 25) — structurally
...
**never auto-resolved to AdamW**: if Muon is meaningfully better, the lead presents the
measured delta and the user owns the ≤1-day relaxation.

## Expected receipt schema (so the engineer's `eng/329c` harness matches; loader is tolerant)
