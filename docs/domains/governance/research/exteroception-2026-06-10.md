...
precedent: the predecessor CLI's custom WebSearch/WebFetch, custom because the assistant model vendor's versions
...
and is the precedent the user named: the predecessor CLI reimplemented WebSearch as an
...
because the model vendor's versions are *server-side tools*
