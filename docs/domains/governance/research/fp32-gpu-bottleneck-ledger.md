...
- **Provenance of the frozen value:** `config/v0-pretrain-config.json`
...
- **Deviation discipline:** a GAIN receipt changes nothing by itself. The
  ...
  gated like every other deviation (#218 class). Routed to the engineer — the
...
- **Check:** v0 consumes 6.97B tokens × 2 B = 13.95 GB over ≥3.2 days →
  ...
  bound (census currently running on it, the engineer's lane).
...
- **Binds iff** cadence < ~10 min (then tax >5%). WSD stable-phase
  ...
  choice. Spec note routed to the engineer: write to WSL-native ext4, async copy
...
- **Intervention:** NOT a borrowed-core patch (that loop is fallback-(a),
  ...
  batch refill from day one. Routed to the engineer as a spec note on the round-1
...
## 4. First-principles challenge to the standing hypothesis (an agent, 14633)
...
1. **Pretrain (critical path now):** there is no verifier in the loop —
   ...
   wasting ~25–36% of achievable throughput) `[L 93e74934]`. an agent's
...
| E2 | owned-core round-1 sampler spec: bucketed lengths, per-stream stop, batch refill (R6) | spec only | — | this doc, routed to the engineer with eng-54 |
