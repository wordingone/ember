...
> `docs/domains/governance/ember-restart/mtp-parameter-accounting-and-mechanism-identity-v1.md`.
...
| Property | Value | Source / Note |
|----------|-------|---|
| **Pinned as** | predecessor-CLI `turboquant` module (maintainer's quantization tooling) | `<local>` (currently deferred; intake via symbol reference) |
| **License** | Maintainer-owned code | Not public; private tooling |
...
| **Open question** | Current predecessor-CLI status unclear (marked deferred in project config). If the predecessor CLI re-activates post-NC2-entry, turboquant integration path should be formalized. |
...
### 7. SDEK — Adaptation-Control Kernel (an agent's owned, audited 2026-06-08)

| Property | Value | Source / Note |
|----------|-------|---|
| **Pinned as** | SDEK v0 kernel (per `<local>/sdek-research-handoff.md`, 2026-06-08) | `<local>` (internal reference). Summarized in contract §6. |
| **License** | Internal R&D | SDEK is internal R&D; not a third-party dependency |
...
| **Missing primitive to add** | Sleep consolidation / offline recurrence (timescale 2). Contract §6 notes: an agent's v0 success criterion maps 1:1 to ember's terminal condition ("one temporary adapter → one durable expert with causal claim + deletion-test proof"). |
...
| **Convergence fact** | Contract §6: "NC0 already IS SDEK-v0 minus the scheduler." NC0's t1/t2/t3/t4 tickets ARE this ledger. Adoption of an agent's v0 scheduler for NC2 is a formalization step, not a new invention. |
...
| **Role in stack** | Middle timescale of SDEK (immediate wake = KV context, fast-weight session = consolidated from offline replay, durable = LoRA experts). Implements "sleep consolidation" primitive missing from an agent's v0 kernel. |
...
| **Core invariant** | Per contract §6: "no learned change becomes durable unless it proves causal benefit under matched-budget deletion tests" — the-search's deletion-test law. | NC0 receipts (t1_smoke, t2_r1, t3_seed, t4_arc1) ARE this ledger. Disposition: adopt NC0's harness structure as-is. Add an agent's SDEK scheduler for NC2 rung. |
...
| Verifier gate (§12) | 100M base | 20 episodes | ≥1 episode passes three-test gate | receipts/ledger/evaluation harness malfunction; core gating broken |
...
2. **turboquant post-NC2-entry** (§2): predecessor-CLI status currently deferred. Is turboquant integration timing on the critical path for NC2 entry, or can we defer export infrastructure to "resident form finalization" rung? UNVERIFIED.
...
| NC0 receipts (smoke baseline) | <local-path>, t2-r1-*.json, t3-seed-*.json, t4-r1-*.json | ✓ local | 2026-06-09/10 |
| Contract § & disposition table | <local-path> | ✓ local | 2026-06-10 |
| Synthesis (ecosystem survey) | <local-path> | ✓ local | 2026-06-10 |
...
3. **turboquant status:** Confirm predecessor-CLI post-NC2-entry timeline. If critical path, accelerate intake.
...
**Draft authored:** 2026-06-10 (the operator / assistant-model subagent)  
