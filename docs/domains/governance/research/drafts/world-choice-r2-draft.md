...
- Publicly available since 2023 (no copyright ambiguity; OpenAI-aligned corpus). **Passes §7(c) — eval split survives test-set contamination checks** (this is EvalPlus's published design intent).
...
**Source:** Vault corpus (in-flight, under the engineer collection). IFC files (Industry Foundation Classes, ISO 16739-1:2018). Verification = schema compliance + geometry round-trip checks (NURBS kernel checks, building-footprint closure, roof connectivity).
...
**Floor Accessibility — 3B Baseline:**
- **NO MEASURED BASELINE.** IFC-writing is not a standard LLM benchmark. Vault corpus is still in L1 (raw collection phase; the engineer is collecting, the lead gates).
...
- Directly couples to the engineer's vault collection work (L1–L5 manifest structure).
...
- **Justification:**
  ...
  - Blocking prerequisites: Vault L1 finish (the engineer), native IFC verifier instrumentation (WEB-CAD side).

- **Re-assess:** After the engineer completes Vault L1 manifest + the lead gates it. Then build harness + run pre-commit probe (30 IFC files, 50-line tasks, pass_any_pct ≥5% threshold) in NC1c round.
...
2. **ARC-AGI-3 harness complexity:** The t4_arcade.py build is non-trivial (SDK integration, action-space mapping, deterministic seeding). Should this be the engineer's job (engineering) or pre-built as a script asset? (Recommendation: the engineer task, wrapped in scripts/ for reusability across policy experiments.)

3. **Vault L1 completion deadline:** NC1c admission of W-ifc is gated on Vault L1 finish. Current the engineer ETA? (Use this to backplan NC1c probe timing.)
