...
| G-budget | Launch date L satisfies: days-remaining(L→the maintainer 22) ≥ 4.55 (receipted-unstacked envelope, fp19-bench) — i.e. **L ≤ the maintainer 17** unstacked, L ≤ the maintainer 19 if the conservative stack (3.12 d) is receipted locally first | GREEN today (11 days; tightens daily) |
...
The v0 pretrain shim (`src/ember/governance/scripts/v0_pretrain_launch_gate.py`, **BUILT**) embeds
...
1. the engineer's #160 production freeze receipt (interlocked behind the #149 gate
   — already merged, so unblocked on his side) → G-tokenizer.
2. G-config build (the lead or gated subagent): config file + selftest +
...
*Owner: the lead. Refs: #166 (fp-26), #139 (fp-24), #160 (tokenizer), task #49.*
