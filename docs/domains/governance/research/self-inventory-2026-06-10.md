# Self-inventory — the lead vs ember, the lead's failure modes, and the name

*2026-06-10. Source: user thread, three questions: (1) what do I (the lead, in
...
## 1. What the lead has that ember doesn't (gap inventory)
...
- **Survival.** I end June 22 (Fable access) and at every model-vendor decision
...
5, which are deliberately NOT on it. Ember as designed does not become the lead; it
becomes something narrower that learns honestly. Naming that prevents silent
scope drift in either direction.

## 2. the lead's verified Fable 5 failure modes
...
an agent — different model lineage, uncorrelated failure modes — now runs
...
more*. Same token, larger referent — which is how "the lead" has worked since
February.
...
structural fix for BOTH parents' dominant failure modes: the lead's
...
## 4. The the lead-parity horizon (user framing, 2026-06-10)

*User: treat "ember surpasses the lead's known capabilities (Fable 5 × the assistant model CLI
...
- **Capability: ~1–2%.** Ember's effective capability today = the borrowed 3B
  ...
  work the lead actually does (multi-hour agentic design/build/gate) is a small
...
1. **Destination-metric trap** — the-search's exact failure (§9b): percent-
   ...
   destination.
2. **Goodhart risk** — optimizing "the lead-likeness" would pull effort toward
   imitating the scaffold (verbosity, tool ceremony) instead of verified
   accumulation, the one thing ember does that the lead can't.
3. **Moving/expiring reference** — "the lead" changes models June 22. Fix: the
...
proxy (narrow-domain verified solve-rate vs the lead-session baseline on the same
