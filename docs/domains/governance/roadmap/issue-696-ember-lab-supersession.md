...
|---|---|---|
| First real consumer starts the service lazily, never at boot | `src/ember/infrastructure/tools/ember-cli/src/commands/model.ts` and `entrypoints/process-entry.ts` call `entrypoints/owned-server-supervisor.ts` only while a bound model command/session is established (`REORGANIZED`) | Ordinary unbound boot is refused before the direct-spawn compatibility branch; only an explicit `REFERENCE_ONLY` seat may use that non-owned compatibility path. |
