...
`src/ember/governance/scripts/branch_inventory.py` is the standing, read-only inventory for local
...
```powershell
python -B src/ember/governance/scripts/branch_inventory.py capture `
  --repo . `
...
```powershell
python -B src/ember/governance/scripts/branch_inventory.py check `
  --repo . `
  --master refs/remotes/origin/master `
