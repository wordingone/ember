...
kill criteria) + `docs/domains/governance/spec/ember02-preregistration-thresholds-v1.json` (T-01..T-09, F-11).
...
| **R1-E5** | first closed-boundary frontier receipt (§5.4) with `energy_boundary: DEGRADED_PROXY` | No frontier receipt anywhere. No energy-proxy sampling occurred (`src/ember/governance/scripts/energy_proxy_logger.py` exists but was not invoked). No frontier-receipt generator script exists anywhere in `scripts/` (repo-wide grep, zero hits). The §5.2 fixed-prior manifest **does** exist and is hash-checkable (`manifests/ember-restart-3b/fixed-prior-manifest-v1.json`) — one of §5.4's 8 field classes, not the receipt itself | **EVIDENCE-MISSING** |
