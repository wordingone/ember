...
| DT-6 | loop-economics gate explicit (an agent 15051) — a diagnostic PASS must read as verified-signal-per-GPU-hour, not "it ran"; the equal-wall-clock band IS that metric — make it explicit in the verdict prose | prereg amendment | the lead | **DONE** (docs/archive/pre-restart/dt6-loop-economics-gate-amendment.md, 2026-06-14; checker src/ember/governance/scripts/loop_econ_gate.py = eng successor → the engineer) |
