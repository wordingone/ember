...
| Branches and deletion | A branch is never removed from age or title alone; open-PR, ancestry, protection, and external-reference checks are required. | `git ls-remote --heads origin`, PR-head census, compare-to-master proof, retained deletion manifest. | NO_SAFE_CANDIDATE_IN_SCREENED_SAMPLE (3); zero remote mutations. |
