...
| CLI model identity | `src/ember/infrastructure/tools/ember-cli/src/model-config.ts:5-6` hardcodes `LOCAL_MODEL_ID = 'qwen-3.6'`. | Feature checks treat that string as the local model. | Borrowed Qwen is encoded as the local identity. | Replace with manifest-derived disposition/model ID; make Qwen an explicit reference seat. |
| CLI config source | `src/ember/infrastructure/tools/ember-cli/src/entrypoints/process-entry.ts:98-108` defines endpoint, binary, model path, and modelName independently. | `loadModelsJson` returns null for missing, unreadable, or malformed config at lines 146-152. | Config loss silently falls through; no manifest/hash identity exists. | Require a manifest reference and fail closed when identity is absent or invalid. |
...
| Session endpoint fallback | `src/ember/infrastructure/tools/ember-cli/src/entrypoints/session-init.ts:520-539` falls back to environment then `http://localhost:8081`. | A reachable endpoint becomes the production client at lines 593-596. | Default location can silently supply a different backend. | Remove identity-free fallback; require bound endpoint or explicit offline state. |
| Model lifecycle | `src/ember/infrastructure/tools/ember-cli/src/services/model-lifecycle.ts:29-32` stores process state in module globals. | External endpoints are declared unmanaged at lines 64-72 and 98-106. | New CLI instances cannot recover durable ownership or identity. | Move lifecycle/process identity to `ember-lab`; manifest remains the model authority. |
| Historical 2.2B brain supervisor registry (retired) | The former `services/brain-server-supervisor.ts` mirrored port/path/PID/launcher/time/device. | Its PID-reuse and stale-row behavior belonged to the removed borrowed vehicle. | It was not bound to current checkpoint, executable, protocol, or manifest identity. | Losslessly transferred to Ember Lab's daemon-owned job/lease/health/event receipt; `entrypoints/owned-server-supervisor.ts` is the lazy CLI consumer and never authors authority. |
| Historical brain default checkpoint (retired) | The former supervisor derived a fixed model path and treated it as informational. | Child resolution could disagree with the parent. | A fixed 2.2B path cannot identify a current served subject. | Current Ember Lab dispatch binds the selected manifest/receipt and checkpoint bytes; no fixed path or old vehicle remains an authority. |
| Serving registry | `src/ember/governance/scripts/serving_registry.py:2-19` declares itself the model-server source of truth with row schema `port, model_path, pid, launched_by, ts, device`. | Missing registry returns an empty set; writes are atomic. | It is a process locator, not a model identity registry. | Retain as derived compatibility output only; `ember-lab` is sole runtime writer. |
| Owned cbase server label | `src/ember/governance/scripts/serve_cbase_openai.py:73-106`, `:520`, and `:540` hardcode `cbase-2.2b`. | The same label is returned regardless of verified served/trained capacity. | Label exceeds/obscures the actual historical object described in `STATE.md:68`. | Remove fixed identity; serve only the verified manifest model ID/disposition. |
...
| Dry-run accounting | `src/ember/governance/scripts/cbase_grow_rung2_dryrun.py:315-370` loads model/optimizer/RNG and computes before/after and alias-adjusted counts. | Load/check calculations can pass on a dry run. | Dry-run allocation can be reported beside training language without served/trained identity. | Bind dry-run disposition and prohibit model-birth/capability credit. |
| Benchmark admission | `src/ember/governance/scripts/ember_research_benchmark_harness.py:15-21` freezes five IDs; lines 65-111 validate assets and emit admission metadata. | Admitted means harness/tasks are ready. | Receipt lacks exact subject checkpoint, comparator, score uncertainty, and owned-completion boundary. | Require evaluation section from the subject manifest; readiness never increments completion. |
| Cockpit restart/outage | `src/ember/infrastructure/tools/ember-cli/src/services/windows-restart-task.ts` registers the exact hashed Ember executable with a least-privilege Windows restart-on-failure task; model-server launch remains exclusively manifest-bound through `ember-lab`. | The OS may restart only the exact installed Ember cockpit binary after failure; it has no server-launch authority. | A stale task registration can point at old Ember bytes. | Registration rehashes the executable before/after, verifies installed task XML, and writes a path-free receipt; rerun registration after an Ember binary update. |
| Operator display | `src/ember/infrastructure/tools/ember-cli/src/screens/repl.ts:887-891` renders endpoint/circuit/outage state. | UI can show health and outage without exact loaded subject. | Healthy/offline is observable; owned/reference identity is not. | Render model ID, disposition, checkpoint prefix, backend hash, lease, and unresolved state. |
