...
## CIA expert object boundary in the checkpoint module

`checkpoint_artifacts.read_cia_expert_object` requires an externally pinned index digest and selects a hash-addressed object from its closed 25-expert identity map. It deserializes the same bounded byte snapshot whose digest was verified, then checks the full selected expert inventory, BF16 CPU dtype, finite values, exact backing storage and non-overlapping byte ranges. It returns tensors without mutating a model. Validation of the selected object is not validation of every other object or admission of a complete generation.

`checkpoint_artifacts.write_cia_expert_object` requires an existing candidate under the checkpoint quarantine and the current process's writer lease. It validates the full expert payload, serializes through the existing scratch-byte cap and creates the content-addressed object through the existing durable no-replace primitive. Repeated creation cannot overwrite the old object; changed bytes create a different identity. This helper does not create another checkpoint ledger or replace the existing complete-bundle publication transaction.

The object tests reopen a full 113,246,208-element expert, reject a substituted valid tensor file, wrong index and payload identities, reused object identities, missing objects, undeclared backing storage and overlapping buffers. They also exercise byte-cap refusal, lease refusal and immutable object creation. No smaller learning model is constructed. Complete generation-index publication, integration into the full checkpoint writer/load dispatcher, optimizer/RNG/cursor recovery and guarded activation remain outstanding.
`checkpoint_artifacts.write_cia_expert_index` freezes a closed 25-object index, checks an explicit aggregate byte allowance, validates every selected object through the digest-pinned reader, rechecks the existing writer lease and creates a hash-addressed index without replacement. It retains only one deserialized expert at a time. Failure removes the private draft; it does not publish a partial final index. The index is a component of a candidate checkpoint, not a replacement for its core, optimizer, RNG, cursor, qualification or activation bindings.

The opt-in full-index test initializes the complete 3,082,539,008-parameter CPU model, writes all 25 expert bundles, publishes the verified index and reopens all 2,831,155,200 expert elements for exact comparison. Its temporary object bank is bounded below 6 GiB and removed on normal completion. This does not establish full-checkpoint recovery or retain a learned generation.
### Immutable shared component

`checkpoint_artifacts.write_cia_core_object` and `read_cia_core_object` preserve all shared core, router and adapter tensors, including the tied vocabulary matrix, under an exact validated architecture digest. They reuse the existing checkpoint quarantine, writer lease and durable no-replace primitive, and share physical tensor validation with expert objects. The reader hashes and deserializes the same bounded snapshot. Its record must come from the verified complete checkpoint; the component reader is not another admission authority.

A fixed non-learning 251,383,808-element component roundtrip preserves exact values and rejects missing router state, insufficient byte allowance, replacement, wrong architecture binding, substituted valid tensor files and wrong payload identity. Together with expert-object tests, this is component custody evidence only. Core and expert objects still need one complete transaction binding optimizer/master state, RNG, cursor and the existing qualification/admission authority before recovery or serving credit.
