...
`census_consumers.py` across seven logical public/private/live-local roots.
...
The checked snapshot contains 25,402 evidence records: 22,679 executable-source
line records and 2,723 content-hashed file/category records for state, config,
...
evidence surfaces, and the
configured-but-missing private backup root. The canonical snapshot is
22,671,446 bytes with SHA-256
`b9bbe5fb8703c86993a73cbf95183a26a27868ab558015a1a61209a68cde1366`.
...
| Model lifecycle | `tools/ember-cli/src/services/model-lifecycle.ts:29-32` stores process state in module globals. | External endpoints are declared unmanaged at lines 64-72 and 98-106. | New CLI instances cannot recover durable ownership or identity. | Move lifecycle/process identity to `emberd`; manifest remains the model authority. |
| Brain supervisor registry | `tools/ember-cli/src/services/brain-server-supervisor.ts:71-79` mirrors port/path/PID/launcher/time/device. | Stale rows are reclaimed using PID/cmdline checks at lines 431-450. | No checkpoint, executable, protocol, or manifest hash is recorded. | Extend the compatibility view from the bound `emberd` identity; never author identity here. |
...
- [ ] Derive the serving-registry compatibility view from `emberd`; remove dual writers.
