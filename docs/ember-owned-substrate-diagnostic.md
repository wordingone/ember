...
Opened 2026-06-13 ~14:47Z on the maintainer directive: ember has been too SHALLOW —
...
> **[DT-1 PROCEED EVIDENCE VOID 2026-07-10]** The DT-1 delta-rule PASS receipt (dt1-delta-rule-20260616T165810Z) is void as learning/GDN evidence per errata issue #670 (chance-level, out-of-band, missing DT-6 gate fields). The PyTorch baseline remains valid; a fresh DT-1 run under the DT-6 gate establishes new proceed evidence.
...
- **Runtime-axis PROOF (#4):** `fluxcore_torch.py` / `selfref/` / `atomic_fold.py`
  ...
  `fluxcore_torch.py` is the engineer's runtime template.
- **Kills guard (#3):** zero-init local rules lock into winner-take-all (13
  ...
  warmup) before switching to the fused update — folded into the engineer's spec (mail
...
NEXT: the engineer runs the warm-init delta-rule LM-block diagnostic (mail 15439/15440);
