...
def write_task18_family() -> None:
    ...
    for i in range(120):
        if i % 3 == 0:
            smiles = "c1ccccc1" + "C" * (1 + (i % 12))
        elif i % 3 == 1:
            smiles = "C" * (5 + (i % 20)) + "O"
        else:
            smiles = "C" * (5 + (i % 20)) + "N"
        fitness = 0.55 + 0.30 * ((i % 17) / 16.0)
        lines.append(f"{smiles},{fitness:.4f},{i % 1000},{i // 12}")
...
def write_task19_family() -> None:
    ...
    for idx in range(11, 18):
        ...
        for j in range(3):
            row = f"{idx},OCCO,C=C,CCOC(=O)OCC{idx}{j}"
            rows.append(row)
            per_rows.append(row)
...
def write_zmat_poscar_family() -> None:
    ...
    poscar = [
        "Ember generated POSCAR",
        "1.0",
        "  9.8765000000  0.0000000000  0.0000000000",
        " -4.9382000000  8.5533000000  0.0000000000",
        "  0.0000000000  0.0000000000  5.9525000000",
        "C",
        str(n),
        "Cartesian",
    ...
    for i in range(n):
        x = (i % 8) * 1.20 + 0.181486
        y = ((i // 8) % 4) * 1.95 + 0.107452
        z = 3.50 + 0.04 * math.sin(i / 3.0)
    ...
    for i in range(n):
        if i == 0:
            ...
            zmat.append(f"C 1 {1.420000 + 0.001 * i:.6f}")
        elif i == 2:
            zmat.append(f"C 2 {1.420000 + 0.001 * i:.6f} 1 120.000000")
        else:
            zmat.append(f"C {i} {1.420000 + 0.001 * i:.6f} {i-1} 120.000000 {i-2} 180.000000")
