...
Last verified: 2026-07-07T05:15Z (UTC).
...
| **body (P-C)** | Cockpit windows closed on operator order 2026-07-07. Relaunch gated by operator-side binding contract: positioned left-half launch, #310 (status-bar highlight) bisect in flight, #307 increment-1 (activity event transport, PR #309 open) live. C(-1) cure path specced: spend-annex full coverage + emitter-side field declaration (forward-only). | #310 bisect complete → #307 increment-2 spec; ember-cli left-half launch on operator order. |
