...
semantic mapping onto docs/domains/governance/authority/GOAL.md board conditions, since receipt payloads can
...
Usage:
    python src/ember/governance/src/ember/governance/scripts/build_claims_index.py [--receipts-dir PATH]
...
DEFAULT_RECEIPTS_DIR = Path(__file__).resolve().parents[4] / "receipts"
...
def render_claims_md(rows) -> str:
    ...
    lines = [
        "# Claims index",
        "",
        "Generated by `src/ember/governance/src/ember/governance/scripts/build_claims_index.py` from the `ticket` / `issue` / "
        ...
        "",
        "```",
        "python src/ember/governance/src/ember/governance/scripts/build_claims_index.py",
        "```",
        "",
