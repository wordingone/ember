...
def load_goal_binding(root: Path) -> tuple[str, str, tuple[str, ...]]:
    ...
    if not isinstance(goal, str) or not goal:
        raise ValueError("docs/domains/governance/authority/GOAL.md active_goal_id missing")
    if not isinstance(outcome, str) or not outcome:
        raise ValueError("docs/domains/governance/authority/GOAL.md next_executed_outcome missing")
    if (
        ...
        raise ValueError("docs/domains/governance/authority/GOAL.md active_workstream_ids invalid")
...
def validate_pr_body(
    ...
    if len(goals) != 1:
        ...
        errors.append("goal_id does not match docs/domains/governance/authority/GOAL.md")
    if len(outcomes) != 1:
        errors.append("next_executed_outcome must appear exactly once")
    elif outcomes[0] != expected_outcome:
        errors.append("next_executed_outcome does not match docs/domains/governance/authority/GOAL.md")
    if len(workstreams) != 1:
        errors.append("workstream_id must appear exactly once")
    elif workstreams[0] not in set(allowed_workstreams):
        errors.append("workstream_id is not allowed by docs/domains/governance/authority/GOAL.md")
    return errors
