...
Launch interlock: requires --the lead-gate-token=<non-empty>. Any dispatch
without this token raises SystemExit(1) before any dataset or training work.

MTP note (eng #140, an agent r2 audit + gate rework): the original
...
wrote — receipts/ledger/views/wcode-r2-sft.jsonl — sha-pinned, so dataset
...
def _require_gate_token():
    ...
    if not args.maintainer_gate_token.strip():
        print(
            "ERROR: t2_r2_mtp.py requires --the lead-gate-token=<non-empty>. "
...
def main():
    ...
    if args.theta is not None or args.all_verified:
        print(
            ...
            "(receipts/ledger/views/wcode-r2-sft.jsonl), where theta/ext-clean were "
    ...
    runpy.run_path(
        f"{NC}/src/ember/governance/scripts/t2_mtp.py",
        run_name="__main__",
    )
