...
"""Admit the frozen #2120 heldout AUDIO set (LibriSpeech test-clean flac bytes) without
consuming any transcript, speaker, or chapter metadata."""
...
CENSUS_SCHEMA = "ember-issue1947-heldout-audio-candidate-census-v1"
CENSUS_RAW_SHA256 = "5f00319f595ef08d4bae6c7f4fb931a790e2b3add202f23d652801fa116e0da2"
CENSUS_SELF_SHA256 = "3a0f19cad44f0dc8b31bc80ac90c44550e3379cff0b2b3dd5702169932f1befb"
SELECTED_SET_SHA256 = "f335061f27342b062df2721dd72cfbc5328532405fbe5bad1357abc9a24f7ab0"
...
SEED_MANIFEST_SHA256 = "a1bbc917b147e1607a8b41a54038a37815d6f9bdc723d3fb7aa040e02657fdc7"
SOURCE_SUBSET = "test-clean"
SOURCE_SELECTOR = "librispeech-test-clean-heldout-audio-64"
CATALOG_SOURCE_ID = "candidate-audio-heldout-0"
SELECTED_COUNT = 64
EXPECTED_FLAC_COUNT = 2620
EXPECTED_UNIQUE_FLAC_COUNT = 2620
EXPECTED_TRANS_FILE_COUNT = 87
EXPECTED_SPEAKER_COUNT = 40
EXPECTED_CHAPTER_COUNT = 87
...
SELECTION_RULE = (
    "first N unique flac byte SHA256 values in ascending lexical order after exact "
    "exclusion of every admitted train audio object SHA256; seed-free"
)
CLAIM_BOUNDARY = (
...
def _verified_census(raw: bytes) -> dict[str, Any]:
    ...
    if (
        ...
        or census.get("flac_count") != EXPECTED_FLAC_COUNT
        or census.get("unique_flac_count") != EXPECTED_UNIQUE_FLAC_COUNT
        or census.get("trans_file_count") != EXPECTED_TRANS_FILE_COUNT
        or census.get("trans_files_read") is not False
        or census.get("speaker_count") != EXPECTED_SPEAKER_COUNT
        or census.get("chapter_count") != EXPECTED_CHAPTER_COUNT
        or census.get("selection_rule") != SELECTION_RULE
        or census.get("selected_count") != SELECTED_COUNT
        ...
        or source.get("subset") != SOURCE_SUBSET
        or source.get("seed_tar_sha256") != SEED_TAR_SHA256
        or source.get("seed_manifest_sha256") != SEED_MANIFEST_SHA256
        or source.get("license_sha256") != LICENSE_SHA256
        or not str(source.get("source_url", "")).startswith("https://")
    ):
...
def build_admission_plan(
    *,
    census_raw: bytes,
    license_raw: bytes,
    admitted_train_audio_hashes: set[str],
    ...
    for row in census["selected"]:
        ...
        payload = payloads_by_member.get(member)
        ...
        if (
            ...
            raise ValueError(f"SELECTED_AUDIO_PAYLOAD_DRIFT_REFUSED:{digest}")
        if digest in admitted_train_audio_hashes:
            raise ValueError(f"TRAIN_HELDOUT_AUDIO_OVERLAP_REFUSED:{digest}")
        if digest in selected_hashes:
            raise ValueError(f"DUPLICATE_SELECTED_AUDIO_REFUSED:{digest}")
        selected_hashes.add(digest)
        if not member.lower().endswith(".flac") or member.lower().endswith(".trans.txt"):
            raise ValueError(f"SELECTED_AUDIO_MEDIA_TYPE_REFUSED:{digest}")
        files.append(
            {
                "path": f"objects/{digest[:2]}/{digest}.flac",
                "bytes": len(payload),
                "sha256": digest,
                "source": {
                    "member": member,
    ...
    if len(files) != SELECTED_COUNT:
        raise ValueError("SELECTED_AUDIO_TOTALITY_REFUSED")
    return {
        "schema_version": "ember-issue1947-heldout-audio-admission-plan-v1",
        "result": "PASS",
        "selection_rule": SELECTION_RULE,
        "selected_count": SELECTED_COUNT,
        "selected_set_sha256": SELECTED_SET_SHA256,
        "train_exclusion_assertion": "executed_pass",
        "admitted_train_audio_object_count": len(admitted_train_audio_hashes),
        "census_raw_sha256": CENSUS_RAW_SHA256,
        "census_self_sha256": CENSUS_SELF_SHA256,
        "license_sha256": LICENSE_SHA256,
        "seed_tar_sha256": SEED_TAR_SHA256,
        "seed_manifest_sha256": SEED_MANIFEST_SHA256,
        "source_url": census["source"]["source_url"],
...
def build_audio_only_contract(
    ...
    if (
        ...
        or plan.get("selected_count") != SELECTED_COUNT
        or plan.get("selected_set_sha256") != SELECTED_SET_SHA256
        or plan.get("transcripts_read") is not False
        or len(plan.get("files", [])) != SELECTED_COUNT
    ):
        raise ValueError("AUDIO_ADMISSION_PLAN_TOTALITY_REFUSED")
    catalog_binding: dict[str, Any] | None = None
    if (catalog_export_raw is None) != (dataset_id is None):
        raise ValueError("AUDIO_CATALOG_BINDING_ARGUMENT_TOTALITY_REFUSED")
    if catalog_export_raw is not None and dataset_id is not None:
        try:
            catalog = json.loads(catalog_export_raw)
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            raise ValueError("AUDIO_CATALOG_EXPORT_UNREADABLE_REFUSED") from error
        ...
        if not isinstance(records, list) or not isinstance(edges, list):
            raise ValueError("AUDIO_CATALOG_EXPORT_SCHEMA_REFUSED")
        if not any(
            isinstance(row, dict)
            ...
            raise ValueError("AUDIO_HELDOUT_DATASET_MISSING_REFUSED")
        membership_ids = {
        ...
        if (
            len(membership_ids) != SELECTED_COUNT
            or len(memberships) != SELECTED_COUNT
            or any(
                row.get("split") != "heldout"
                or row.get("domain") != "audio"
                or row.get("admission_state") != "admitted"
                for row in memberships.values()
            )
            or object_ids != expected_object_ids
        ):
            raise ValueError("AUDIO_HELDOUT_MEMBERSHIP_TOTALITY_REFUSED")
        catalog_binding = {
    ...
    contract: dict[str, Any] = {
        ...
        "task": {
            ...
            "prediction": "sha256(audio_payload_bytes)",
        ...
        "source": {
            ...
            "census_raw_sha256": CENSUS_RAW_SHA256,
            "license_sha256": LICENSE_SHA256,
            "transcript_access": "never_read; transcripts_speakers_chapters_forbidden",
        },
        "selection_rule": SELECTION_RULE,
        "selected_set_sha256": SELECTED_SET_SHA256,
        "frozen_items": [
        ...
        "totality": {"expected": SELECTED_COUNT, "observed": SELECTED_COUNT, "complete": True},
...
def write_admission_artifacts(
    *,
    ...
    connector_files = []
    for row in plan["files"]:
        raw = payloads_by_member[row["source"]["member"]]
    ...
    admission = dict(plan)
    admission["schema_version"] = "ember-issue1947-heldout-audio-admission-receipt-v1"
...
def build_projection_spec(
    *, connector_receipt_path: Path, connector_receipt_raw: bytes,
    ...
    census_path: Path, license_path: Path,
    ...
    spec = {
        ...
        "rows": [{
            ...
            "expected_source_selector": SOURCE_SELECTOR,
...
def read_admitted_train_audio_hashes(catalog: Path) -> set[str]:
        ...
        hashes = {
            ...
            for row in connection.execute(
                ...
                WHERE m.kind = 'membership'
                  ...
                  AND lower(json_extract(o.payload_json, '$.media_type')) LIKE 'audio/%'
                """
            )
        }
    finally:
        connection.close()
    if len(hashes) != EXPECTED_ADMITTED_TRAIN_AUDIO_COUNT:
        raise ValueError(f"TRAIN_AUDIO_CATALOG_COUNT_DRIFT_REFUSED:{len(hashes)}")
    return hashes


def read_selected_payloads(seed_tar: Path, census_raw: bytes) -> dict[str, bytes]:
    """Read ONLY the selected flac members' bytes from the frozen seed tar. Transcript
    members are never opened; the tar's own sha256 is verified before any member read."""
    ...
    wanted = {str(row["member"]) for row in census["selected"]}
    if any(not name.lower().endswith(".flac") for name in wanted):
        raise ValueError("SELECTED_AUDIO_MEMBER_NOT_FLAC_REFUSED")
    ...
    with tarfile.open(seed_tar, "r:gz") as archive:
        for member in archive:
            ...
            if handle is None:
                raise ValueError(f"SELECTED_AUDIO_ORIGIN_MISSING_REFUSED:{member.name}")
            payloads[member.name] = handle.read()
            if len(payloads) == len(wanted):
                break
    missing = wanted - set(payloads)
    if missing:
        raise ValueError(f"SELECTED_AUDIO_ORIGIN_MISSING_REFUSED:{sorted(missing)[0]}")
    return payloads
...
def main() -> int:
    ...
    try:
        ...
        payloads = read_selected_payloads(args.seed_tar, census_raw)
        plan = build_admission_plan(
            census_raw=census_raw,
            license_raw=args.license.read_bytes(),
            admitted_train_audio_hashes=read_admitted_train_audio_hashes(args.catalog),
        ...
        if any(value is not None for value in contract_values):
            if any(value is None for value in contract_values):
                raise ValueError("AUDIO_CONTRACT_ARGUMENT_TOTALITY_REFUSED")
            connector_raw = args.connector_for_contract.read_bytes()
            contract = build_audio_only_contract(
                plan,
                connector_receipt_raw=connector_raw,
        ...
        print(f"error: {error}")
        return 2
    print(json.dumps({"result": "PASS", "selected_count": SELECTED_COUNT, "selected_set_sha256": SELECTED_SET_SHA256}, sort_keys=True))
    return 0
