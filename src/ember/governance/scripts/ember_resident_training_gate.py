...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ember-resident-gate-") as td:
        ...
        blocked = build_gate_receipt(repo, index, None, None, [Path("src/ember/governance/scripts/ember_resident_training_gate.py")], full_parity_path)
...
def main() -> int:
    ...
    changed = [Path(p) for p in args.changed_path] or [
        Path("src/ember/governance/scripts/ember_resident_training_gate.py"),
        Path("src/ember/governance/scripts/ember_resident_training_gate_selftest.py"),
