...
comparison baseline on the MBPP sanitized/validation slice (10 tasks, k=1).

INTENDED model: google/gemma-4-E2B-it (on-disk, 9.54 GiB).
BLOCKER (20260612): transformers 5.2.0 does not include gemma4 architecture
  (CONFIG_MAPPING keys stop at gemma3). No custom modeling .py in snapshot.
  Fix: `pip install --upgrade transformers` in csi-train env. Not done here
  because the train env is shared infra — the lead must approve.

PROXY (this run): Qwen/Qwen2.5-Coder-3B-Instruct (same on-disk model as E2,
  Apache 2.0, 3.086B). Establishes harness plumbing + gives a reference score
  on the validation slice that can be compared once Gemma4 runs.
...
NC_SCRIPTS = "<local-path>"
sys.path.insert(0, NC_SCRIPTS)

RECEIPTS = f"{NC}/receipts"

# Intended: google/gemma-4-E2B-it — BLOCKED by transformers version
INTENDED_MODEL_ID   = "google/gemma-4-E2B-it"
INTENDED_BLOCK      = "transformers 5.2.0 does not support model_type=gemma4; no .py in snapshot"

# Proxy: Qwen2.5-Coder-3B (on disk, proven from E2)
PROXY_MODEL_PATH = (
    "<local-path>"
    "models--Qwen--Qwen2.5-Coder-3B-Instruct/snapshots/"
    "488639f1ff808d1d3d0ba301aef8c11461451ec5"
)
PROXY_MODEL_ID = "Qwen/Qwen2.5-Coder-3B-Instruct"
...
def main():
    ...
    print(f"[E3] PROXY model: {PROXY_MODEL_ID} (gemma4 blocked)", flush=True)
    print(f"[E3] block reason: {INTENDED_BLOCK}", flush=True)

    tok = AutoTokenizer.from_pretrained(PROXY_MODEL_PATH, trust_remote_code=False)
    t_load0 = time.perf_counter()
    model = AutoModelForCausalLM.from_pretrained(
        PROXY_MODEL_PATH, dtype=torch.bfloat16, trust_remote_code=False,
    )
    model = model.cuda()
    model.eval()
    ...
    receipt = {
        ...
        "ts": ts,
        "intended_model": {
            "id": INTENDED_MODEL_ID,
            "status": "BLOCKED",
            "block_reason": INTENDED_BLOCK,
            "fix": "pip install --upgrade transformers in csi-train env; the lead approval required",
        },
        "proxy_model": {
            "id": PROXY_MODEL_ID,
            "path": PROXY_MODEL_PATH,
        ...
        "flags": [
            "PROXY RUN: intended gemma-4-E2B-it blocked by transformers version",
            "proxy: Qwen2.5-Coder-3B-Instruct (same model as E2 OOM probe)",
...
def _selftest():
    task_rows = [
        {"task_id": 1, "verified": True},
        {"task_id": 2, "verified": False},
        {"task_id": 3, "verified": True},
        {"task_id": 4, "verified": False},
    ]
    passed = [r["task_id"] for r in task_rows if r["verified"]]
    n_pass = len(passed)
    n_total = len(task_rows)
    pct = round(100.0 * n_pass / n_total, 1)
    assert n_pass == 2 and pct == 50.0 and passed == [1, 3]
    print("FP33_E3_GEMMA_E2B_SMOKE_SELFTEST_PASS")
