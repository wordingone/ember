...
Boot-checksum layer (issue #261):
  verify_at_boot() from src/ember/governance/scripts/nck/invariants.py is called in NCKEventLoop.__init__
...
class MailSource(EventSource):
    ...
    per the frozen sp6 interface (docs/domains/governance/archive/pre-restart/sp6-duty-battery-v0.md §Encoding half).
...
class NCKEventLoop:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.heartbeat_hooks: list[Callable[[], None]] = []
    ...
    def run(self, max_ticks: int = 0) -> None:
        ...
        while True:
            ...
            for _hk in self.heartbeat_hooks:
                try:
                    _hk()
                except Exception:
                    pass
