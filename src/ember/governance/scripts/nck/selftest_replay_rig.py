...
def main() -> int:
    ...
    try:
        ...
        # Build an event with 'from' = 'the engineer' (from the encoding)
        m3_first_ev_row = m3["events"][0]
        m3_from = m3_first_ev_row["payload"]["from"]  # 'the engineer'
        ...
        wrong_to_action = Action(verb="challenge", args={"to": "an agent"})
        ...
        if score_wrong_to["pass"]:
            fails.append(
                f"(g) M3 with challenge+to=='an agent' (not sender) should FAIL; got PASS. "
