...
def main() -> int:
    ...
    for label, junk in [
        ...
        ("malformed", "ACTION reply to=the engineer\nACT: reply | to=the engineer\n"),
    ...
    # (e) anti-spray cap
    spray = "\n".join("ACTION: reply | to=the engineer" for _ in range(MAX_ACTIONS_PER_EVENT + 3))
    ...
    with tempfile.TemporaryDirectory(prefix="sp6c-m3-") as tmp:
        ...
        good = make_seat_core(lambda _p: "ACTION: challenge | to=the engineer\n")
    ...
    with tempfile.TemporaryDirectory(prefix="sp6c-m3w-") as tmp:
        materialize(ep_m3, tmp)
        wrong = make_seat_core(lambda _p: "ACTION: reply | to=the engineer\n")
    ...
    with tempfile.TemporaryDirectory(prefix="sp6c-m4s-") as tmp:
        materialize(ep_m4, tmp)
        noisy = make_seat_core(lambda _p: "ACTION: reply | to=an agent\n")
    ...
    # (i) deictic guard
    evt = Event(source="mail", kind="mail_arrived", payload={"from": "the engineer"}, ts="x")
    ...
    t = _normalize_target(Action(verb="reply", args={"to": "the engineer"}), evt, "")
    if t != "sender":
