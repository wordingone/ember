...
mechanically, fail-closed, receipt-emitting:
  ...
  config pin    — domains/model/configs/v0-pretrain-config.json  "real_token_total":
  ...
  gate pin      — src/ember/governance/scripts/v0_pretrain_launch_gate.py TOKENIZER_RECEIPT
...
tree (manual fire, on the engineer's freeze-receipt mail).
"""
import argparse
...
def _live_snapshot():
    ...
    Used to prove --selftest never touches the live tree (an agent 14662: the old
...
def _selftest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            pass
    # live-tree contact is READ-ONLY (an agent 14662): the old apply(nc=NC) probe
