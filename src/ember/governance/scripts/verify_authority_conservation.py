...
WORKSTREAM_PATH_SCOPES = {'EMBER-02A': {'mode': 'all_except',
 ...
 'EMBER-02C': {'mode': 'only',
               'prefixes': ['docs/ember-restart-eval-',
                            ...
                            'scripts/ember_restart_measured_receipts',
                            'tests/test_ember_restart_measured_receipts']}}
# Single declared transition bridge; the successor PR retires this dual acceptance.
NEXT_WORKSTREAM_PATH_SCOPES = {'EMBER-02A': {'mode': 'all_except',
               'prefixes': ['configs/ember-restart-3b.json',
                            'docs/ember-restart-3b-',
                            'models/ember-restart-3b/',
                            'tools/ember-restart-3b/',
...
AUTHORITY_DIRECTORY = PurePosixPath("docs/domains/governance/authority")
AUTHORITY_LEGACY_DIRECTORY = PurePosixPath("docs/authority")
...
AUTHORITY_MATRIX = "docs/domains/governance/authority/ember-authority-matrix.md"
GOVERNING_SURFACE_MIGRATIONS = {
    "docs/domains/governance/contracts/goal-clear-protocol.md": "docs/contracts/goal-clear-protocol.md",
...
def check_policy(root: Path, policy: dict[str, Any] | None, errors: list[dict[str, Any]]) -> None:
    ...
    expect(
        errors,
        4,
        policy.get("workstream_path_scopes")
        in (WORKSTREAM_PATH_SCOPES, NEXT_WORKSTREAM_PATH_SCOPES),
        ...
        if "ENFORCED" in dispositions:
            if not enforced_by:
                ...
                for target in targets:
                    resolved = target
                    authority_prefix = "docs/authority/"
                    if target.startswith(authority_prefix):
                        name = target[len(authority_prefix):]
                        if name in AUTHORITY_DOCUMENT_NAMES:
                            canonical = (AUTHORITY_DOMAIN_DIRECTORY / name).as_posix()
                            if (root / canonical).is_file():
                                resolved = canonical
                    if resolved == target:
                        for canonical, legacy in GOVERNING_SURFACE_MIGRATIONS.items():
                            if target == legacy and (root / canonical).is_file():
                                resolved = canonical
                                break
                    resolved_targets.append(resolved)
                missing_targets = [
                    target
                    for target, resolved in zip(targets, resolved_targets)
...
def check_execution_only_import_boundary(
    ...
    for row in rows:
        ...
        if not path.is_file() and rel.startswith("scripts/"):
            migrated = root / "src/ember/governance/scripts" / rel[len("scripts/"):]
            if migrated.is_file():
                path = migrated
        if not path.is_file():
...
def check_state(root: Path, errors: list[dict[str, Any]]) -> None:
    ...
    if len(pointer_lines) != 1 or "CONTINUITY.md" not in pointer_lines[0]:
        errors.append(finding(6, "state.pointer_invalid", f"{authority_relative_path(root, 'STATE.md')} must be a one-line docs/domains/governance/authority/CONTINUITY.md pointer"))
    rows = parse_markdown_table(read_text(path), 9)
    if not rows:
        errors.append(finding(6, "state.identity_rows_missing", "no 9-column identity rows in docs/domains/governance/authority/CONTINUITY.md"))
        return
    ids = [row[0] for row in rows]
...
def check_changed_artifact_bindings(
    ...
    *,
    ...
    migration_pairs = (
        ("docs/ember-authority-matrix.md", "docs/domains/governance/authority/ember-authority-matrix.md"),
        ("docs/ember-completeness.md", "docs/domains/governance/contracts/ember-completeness.md"),
        ("docs/registry-dispatch-gate-spec-v0.md", "docs/domains/governance/contracts/registry-dispatch-gate-spec-v0.md"),
        ("docs/nc2-own-technique-contract.md", "docs/contracts/nc2-own-technique-contract.md"),
        ("docs/goal-mode-mechanism.md", "docs/domains/governance/contracts/goal-mode-mechanism.md"),
        ("docs/goal-clear-protocol.md", "docs/domains/governance/contracts/goal-clear-protocol.md"),
        ("docs/goal-live-session.md", "docs/domains/governance/guides/goal-live-session.md"),
        ...
        ("docs/r1-exit-evidence-inventory-20260805.md", "docs/domains/governance/custody/r1-exit-evidence-inventory-20260805.md"),
        ("docs/START-HERE.md", "docs/domains/governance/guides/START-HERE.md"),
        ("docs/PROBLEMS.md", "docs/domains/governance/roadmap/PROBLEMS.md"),
        ("docs/README.md", "docs/domains/governance/DOCS-README.md"),
        ("CONTINUITY.md", "docs/domains/governance/authority/CONTINUITY.md"),
        ("GOVERNANCE.md", "docs/domains/governance/authority/GOVERNANCE.md"),
        ("INVARIANT.md", "docs/authority/INVARIANT.md"),
        ("REDACTIONS.md", "docs/domains/governance/authority/REDACTIONS.md"),
