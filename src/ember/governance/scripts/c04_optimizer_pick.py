...
discipline). The c04 design bench (the engineer) measures each candidate; fp44 measures
...
THE WALL (measured, the engineer c04-receipt3 + dynamo-patch + fp40):
...
THE FROZEN PRECEDENCE (best -> last-resort):
  ...
  PENDING                 batched-NS5 ABSENT-and-still-expected (ns5_pending),
                          ...
                          (the engineer/user ruled batched-NS5 out) lets fp44 decide
...
def pick(batched_ns5_receipt, fp44_score, ns5_pending=True):
    """The frozen precedence. Inputs:
       ...
       ns5_pending: True (default) iff the batched-NS5 bench is still EXPECTED
         to run (the engineer's plan: fp44 first, batched-NS5 second). When True and the
         ...
         the engineer/user has explicitly ruled batched-NS5 out, letting fp44 decide alone.
...
def selftest():
    ...
    # (fp44 lands FIRST in the engineer's dispatch order; this is the live window.)
...
def main():
    ...
    ap.add_argument("--ns5-ruled-out", action="store_true",
                    help="batched-NS5 bench will NOT run (the engineer/user ruled out); "
