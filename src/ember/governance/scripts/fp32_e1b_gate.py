...
The decision rule is frozen in docs/domains/governance/research/fp32-e1b-prereg.md (frozen
...
Hardened per an agent checkpoint 14639 (fail-open surfaces at f3076ae):
...
def check_shard_provenance(pr, nc=NC):
    ...
    if tracked is not None and f"receipts/{name}" not in tracked:
        f.append(f"shard_receipt {name} is not git-tracked — untracked "
                 f"evidence cannot certify a deviation (an agent 14631 class)")
    try:
...
def build_receipt(ts, pr):
    ...
    return {
        ...
        "ts": ts,
        "issue": 225,
        "prereg": "docs/domains/governance/research/fp32-e1b-prereg.md (frozen 2026-06-11 "
                  "pre-shards; contract tightened per an agent 14639)",
...
def _selftest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # --- an agent 14639 repro set: every one must now REFUSE ---
...
def main():
    ...
    if a.selftest:
        ...
        return
    if not a.pair:
        # nonzero on purpose (an agent 14639): the staged state must never be
