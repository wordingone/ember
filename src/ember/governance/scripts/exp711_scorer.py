...
and belongs to treatment-launch time, not this apparatus build):
    python src/ember/governance/scripts/exp711_scorer.py --emit \\
...
Selftest (small synthetic streams, degeneracy kills exercised both ways):
    python src/ember/governance/scripts/exp711_scorer.py --selftest
"""
import argparse
import hashlib
import json
...
def _load_production_tokenizer_from_intervals(intervals_receipt):
    ...
    _ember_95bc16ea56a5e3d6_path = _ember_95bc16ea56a5e3d6_Path(__file__).resolve().parents[4].joinpath('src', 'ember', 'governance', 'scripts', 'exp711_intervals.py')
