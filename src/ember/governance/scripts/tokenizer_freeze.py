...
eng #160 ACs on top of the prep harness:
  - AC-1: the production freeze pins the MERGED eng-36 assembly receipt
    by name AND sha256 (an agent 14560) — absent/drifted = refuse before any
    sampling work;
...
Fail-closed interlocks:
  - refuses to run without manifests/corpus/ + the PINNED assembly
...
Run:  python src/ember/governance/scripts/tokenizer_freeze.py --freeze
...
# eng #160 AC-1 (an agent 14560 assembly-sha gate): the production freeze
...
def load_manifests(repo):
    ...
    if not mirrors:
        raise SystemExit("tokenizer_freeze: no manifests/corpus/ mirrors "
...
def pinned_assembly_receipt(repo):
    ...
    sha256. Absent or byte-drifted -> refuse (an agent 14560)."""
...
def recount_disabled_matching(census_receipt, out_root):
    """eng #195 deviation remedy (the lead 14628, decision frozen pre-census):
    ...
    receipt.update({
        ...
        "deviation_note": {
            "reason": ("text-borne special-token literals: under "
                       ...
                       "(contract-aligned; the lead 14628, decision frozen "
                       "pre-census)"),
...
def main():
    ...
    receipt = {
        ...
        "assembly_pin_basis": ("eng #160 AC-1 (an agent 14560): freeze runs "
