...
# PUBGATE_ENFORCE_FIXTURES_ROOT env var can override. Default: <local-exec-root>/scratch/pubgate-enforce/fixtures
sandbox_root_env = os.environ.get("PUBGATE_ENFORCE_FIXTURES_ROOT")
if not sandbox_root_env:
    raise ValueError("PUBGATE_ENFORCE_FIXTURES_ROOT env var is required. Example: <local-exec-root>/scratch/pubgate-enforce/fixtures")
...
#     exist anywhere in this working tree (<local-exec-root>), only in a separate lane's tree
#     (<private-lineage>) this builder was not authorized to write into or vendor from.
