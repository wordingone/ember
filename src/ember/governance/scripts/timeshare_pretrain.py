...
def _check_launch_interlock(*, live: bool, requested_run: dict | None = None, shard_dir: str | None = None) -> None:
    ...
    # reference: docs/domains/governance/contracts/registry-dispatch-gate-spec-v0.md
...
# ===========================================================================
# Implements the FROZEN contract domains/model/configs/v0-pretrain-config.json against the
...
class PackedShardLoader:
    ...
    def __init__(self, shard_dir: str, seq: int, n_mtp: int, *,
        ...
        _ember_d46db7ae0cab2934_path = _ember_d46db7ae0cab2934_Path(__file__).resolve().parents[4].joinpath('src', 'ember', 'governance', 'scripts', 'fineweb_exclusion.py')
...
def build_v0_model(cfg: dict, *, live: bool, tiny_dims: dict | None = None,
    ...
    class _V0Real(torch.nn.Module):
        ...
        def backbone(self, ids, inputs_embeds=None, span_boundaries=None):
            """v0 backbone forward.

            Multimodal path (Lock 2+3 — eng-36/#420):
              inputs_embeds: (batch, n_image_tokens, hidden) from VisionEmbedder.
              span_boundaries: list of (start, end) image-token spans in the sequence.
              splice_soft_tokens() overwrites embed_tokens rows at span positions.
              build_span_mask() constructs the bidirectional mask (full Lock 3 requires
              EmberMultimodalAttention; LlamaModel uses its own causal mask here).
            Text-only path: inputs_embeds=None → pure causal, unchanged from v0.
            """
            if inputs_embeds is not None or span_boundaries:
                from ember_model_v0_multimodal import splice_soft_tokens, build_span_mask
                h = splice_soft_tokens(
                    self.backbone_model.embed_tokens, ids, inputs_embeds,
                    span_boundaries or [],
                )
                # Lock 3: span mask built here; full bidirectional enforcement
                # requires EmberMultimodalAttention wired into the decoder stack.
                _mask = build_span_mask(
                    h.shape[1], span_boundaries or [], device=h.device
                )
...
def run_v0_segment(
    ...
    *,
    ...
    receipt: dict[str, Any] = {
        ...
        "segment_id": segment_id,
        "contract": "domains/model/configs/v0-pretrain-config.json",
...
def _selftest_v0ext() -> None:
    ...
    _ember_c8d1d1af4ec66d2d_path = _ember_c8d1d1af4ec66d2d_Path(__file__).resolve().parents[4].joinpath('src', 'ember', 'governance', 'scripts', 'v0_config_check.py')
...
def _historical_only_refusal() -> None:
    raise SystemExit(
        "historical_only: the sub-3B cbase trainer is execution-denied"
    )
