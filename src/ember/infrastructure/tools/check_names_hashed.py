...
src/ember/infrastructure/tools/check_line_endings.py: under REPO_GUARD_SCOPE=staged (what
...
DEFAULT_NAMES_EXCLUDE = Path("src/ember/infrastructure/tools/repo-guard-names-exclude.cfg")
...
def load_names_exclude_prefixes(path: Path) -> list[str]:
    """src/ember/infrastructure/tools/repo-guard-names-exclude.cfg: path-prefixes (one per line) exempt
...
def run_generate(plaintext_path: Path, out_path: Path) -> int:
    ...
    header = (
        ...
        "#   python src/ember/infrastructure/tools/check_names_hashed.py --generate "
...
def main() -> int:
    ...
    ap.add_argument("--names-exclude", default=None, help="path-prefix exclusion list (default: src/ember/infrastructure/tools/repo-guard-names-exclude.cfg)")
    ap.add_argument(
        "--scan-guard-surfaces",
        action="store_true",
