...
"""Issue #2167 (W5): paired matched-loss canary for the FP8 shared up+gate site class.
...
between two source heads; W5 binds both heads to the SAME rebased head and alternates the
forward kernel at the 14 installed ``layers.{i}.shared_ffn.up_gate`` sites instead
...
Hooks installed on the base harness (all module-global lookups, patched by name):
  * ``allocate_semantic_runtime``: install the W5 sites after allocation, arm ``bf16``.
  ...
  * ``write_receipt``: bind the ``w5_fp8`` block (installation receipt, per-arm dispatch
...
def arm_kernel(arm: str) -> str:
    if arm not in W5_ARM_KERNEL:
        raise ValueError(f"W5_ARM_REFUSED:{arm}")
    return W5_ARM_KERNEL[arm]


def install_w5(model: Any, *, acceleration: Any | None = None, **install_kwargs: Any) -> dict[str, object]:
    """Install the W5 scope on ``model`` and park every site on the ``bf16`` arm."""

    acceleration = acceleration or _acceleration()
    receipt = acceleration.install_fp8_up_gate_projections(
        model, installation_scope=W5_SCOPE, **install_kwargs,
    )
    if receipt.get("installed_sites") != W5_SITE_COUNT or receipt.get("fallbacks") != 0:
        raise RuntimeError("W5_INSTALLATION_RECEIPT_REFUSED")
    if acceleration.set_fp8_arm(model, "bf16") != W5_SITE_COUNT:
        raise RuntimeError("W5_SITE_COUNT_REFUSED")
...
def run_update_w5(*, acceleration: Any | None = None, base_update: Any | None = None, **kwargs: Any):
    """One base update under the W5 arm switch; refuses cross-arm dispatch and fallbacks."""
    ...
    if _STATE["installation"] is None:
        raise RuntimeError("W5_SITES_NOT_INSTALLED_REFUSED")
    if acceleration.set_fp8_arm(model, kernel) != W5_SITE_COUNT:
        raise RuntimeError("W5_SITE_COUNT_REFUSED")
    ...
    if fallbacks_after != fallbacks_before:
        raise RuntimeError(f"W5_FP8_FALLBACK_REFUSED:{arm}:{fallbacks_after - fallbacks_before}")
    dispatched = after - before
    if kernel == "bf16" and dispatched:
        raise RuntimeError(f"W5_CONTROL_ARM_DISPATCHED_FP8_REFUSED:{dispatched}")
    if kernel == "fp8" and not dispatched:
        raise RuntimeError("W5_TREATMENT_ARM_DISPATCHED_NO_FP8_REFUSED")
    _STATE["arm_fp8_dispatches"][arm] += dispatched
    _STATE["arm_updates"][arm] += 1
    return row, cursor


def w5_block() -> dict[str, object]:
    return {
        "schema_version": W5_SCHEMA_VERSION,
        "issue": W5_ISSUE,
        "installation_scope": W5_SCOPE,
        "site_count": W5_SITE_COUNT,
        "arm_kernel": dict(W5_ARM_KERNEL),
        "arm_switch": "fp8_kernel_at_14_shared_up_gate_sites_same_head_same_process",
...
def bind_w5_receipt(value: Mapping[str, object]) -> dict[str, object]:
    bound = dict(value)
...
def _allocate_w5(**kwargs: Any):
    config, model, optimizer, governor, memory = _ORIGINAL_ALLOCATE(**kwargs)
    install_w5(model)
    return config, model, optimizer, governor, memory


def _update_w5(**kwargs: Any):
    return run_update_w5(**kwargs)


def _configure_w5(**kwargs: Any) -> None:
    if kwargs.get("aa_mode"):
        raise ValueError("W5_AA_MODE_REFUSED")
    if kwargs["control_rebased_head"] != kwargs["treatment_rebased_head"]:
        ...
        raise ValueError("W5_HEADS_MUST_MATCH_REFUSED")
...
def main() -> int:
    if "--aa" in sys.argv:
        raise ValueError("W5_AA_MODE_REFUSED")
    ROBUST.configure_base = _configure_w5
    return ROBUST.main()
