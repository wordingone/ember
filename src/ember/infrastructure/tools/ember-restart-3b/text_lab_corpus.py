...
def adapt_pdf_extraction_receipt(
    *,
    ...
    producer_path = Path(__file__).resolve().parents[1] / "corpus_connectors" / "pdf_to_utf8.py"
...
def _partition_producer_supersession_allows_v2(
    ...
    """Accept only the reviewed four-hop chain for the exact nine bound rows."""
    try:
    ...
    if not isinstance(transitions, list) or len(transitions) != 4 or not isinstance(bindings, list) or len(bindings) != 9:
        return False
    ...
    if (
        ...
        return False
    fourth_basis = (
        "The issue #898 02B carrier moves the producer from its legacy tools path to the "
        "canonical infrastructure namespace and rewrites only its exact-local import seam; "
        "the partition validation and refusal contract is unchanged."
    )
    fourth = transitions[3]
    fourth_fields = {
        "authorization_basis", "migration_authority", "successor_sha256",
        "superseded_sha256",
    }
    expected_migration = {
        "schema_version": "ember-exact-path-migration-binding-v1",
        "old_path": _PARTITION_PRODUCER_LEGACY_PATH,
        "old_sha256": "82443ad203a47abaca9c8f54d1ee09f51029dd0ebbe7a039a802e286855f33c2",
        "new_path": _PARTITION_PRODUCER_PATH,
        "new_sha256": current_sha,
        "reason": "canonical-import migration under 02B",
    }
    if (
        not isinstance(fourth, dict)
        or set(fourth) != fourth_fields
        or fourth.get("authorization_basis") != fourth_basis
        or third.get("successor_sha256") != fourth.get("superseded_sha256")
        or fourth.get("successor_sha256") != current_sha
        or fourth.get("migration_authority") != expected_migration
    ):
        return False
    start = next(
    ...
    if os.name == "nt":
        kwargs["creationflags"] = subprocess.CREATE_NO_WINDOW
    for transition in transitions[start:3]:
        ...
        cause_blob = subprocess.run(
            ["git", "-C", str(repo_root), "show", f"{cause}:{_PARTITION_PRODUCER_LEGACY_PATH}"], **kwargs
        )
        if (
        ...
        for commit in resolved:
            ...
            blob = subprocess.run(
                ["git", "-C", str(repo_root), "show", f"{commit}:{_PARTITION_PRODUCER_LEGACY_PATH}"], **kwargs
            )
            if (
